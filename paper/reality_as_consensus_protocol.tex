...
\begin{abstract}
This paper gives the consensus-theoretic core of Observer-Patch Holography (OPH). If physics is built from local observer descriptions that must agree on overlaps, the first question is whether different repair orders lead to different worlds. On the fixed-cutoff OPH collar branch, the local repair step is not left as an abstract rewrite primitive: it is read from exact Markov splice on exact collars or from declared Petz/Fawzi--Renner reconciliation on recoverable collars. The main result is that, once repair completeness and local confluence hold for those accepted local recovery moves, and once the OPH inconsistency potential \(\Phi\) strictly decreases along that declared branch, the repair dynamics terminates and has a unique schedule-independent normal form. The paper also identifies the loop obstructions to global consistency, shows why physical uniqueness belongs on the gauge quotient rather than on raw microscopic representatives, and gives a fixed-point account of stable classical records. This is the finite patch-net theorem package that supports the broader relativity, gauge, particle, and observer branches in the OPH suite.
\end{abstract}
...
completeness and local confluence hold on the declared recovery branch and once the OPH
inconsistency potential \(\Phi\) strictly decreases along every accepted repair, every initial
configuration has a unique schedule-independent normal form. The second layer concerns obstructions: pairwise overlap agreement does not ensure a
...
\begin{enumerate}
\item \textbf{Asynchronous confluence.} Under repair completeness, strict decrease of the OPH inconsistency potential \(\Phi\), and local confluence, every initial state has a unique normal form, independent of update schedule (Theorem~\ref{thm:confluence}).
...
the OPH inconsistency potential \(\Phi\), and satisfy local confluence, and on the Petz branch
one must verify the support/CPTP clause stated later in Proposition~\ref{prop:petz-cptp}. On the
finite patch net, that Lyapunov descent clause is the step from which termination is discharged.
...
The theorem package separates the remaining theorem-local inputs cleanly:
...
\begin{assumption}[Lyapunov descent on the declared repair branch]\label{ass:lyapunov}
Every enabled repair strictly decreases the inconsistency potential: $s\to t \implies \Phi(t)<\Phi(s)$.
\end{assumption}

On the finite patch-net branch, this makes \(\Phi\) the OPH Lyapunov functional for the accepted
repair dynamics.

\begin{proposition}[Termination from the OPH Lyapunov functional]\label{prop:lyapunov-termination}
Under Assumption~\ref{ass:lyapunov}, every repair sequence is finite; equivalently, the repair
relation \(\to\) is terminating.
\end{proposition}
...
repair step \(s\to t\), Assumption~\ref{ass:lyapunov} gives \(\Phi(t)<\Phi(s)\). An infinite repair
...
Under Assumptions~\ref{ass:complete}, \ref{ass:lyapunov}, and \ref{ass:confluence}, every initial state $s\in\Sigma$ has a unique normal form
\[
...
If one reduction has length~0, there is nothing to prove. Otherwise write $s\to s_1\to^* t$ and $s\to s_2\to^* u$. By local confluence, there exists $w$ with $s_1\to^* w$ and $s_2\to^* w$. Since $s\to s_1$ and $s\to s_2$, Assumption~\ref{ass:lyapunov} gives $\Phi(s_1)<\Phi(s)$ and $\Phi(s_2)<\Phi(s)$. The induction hypothesis applies at $s_1$ and $s_2$. Hence $t$ and $w$ have a common descendant, and $u$ and $w$ have a common descendant. Therefore $t$ and $u$ have a common descendant.
...
recovery-derived local moves satisfy repair completeness together with the Lyapunov
\(\Phi\)-descent and local-confluence hypotheses on the declared branch, and the support/CPTP
clause on the Petz branch. The point here is also that no extra gauge-covariance axiom is needed
once repair is formulated on the quotient.
...
\begin{enumerate}
\item \textbf{Computational complexity.} What is the complexity of computing $\operatorname{nf}_\lambda(s)$? The Lyapunov termination bound from \(\Phi\)-decrease is a worst-case measure, but typical instances may converge much faster. Is there a polynomial bound for natural classes of patch nets?
...
\item \textbf{Recovery-channel control.} The fixed-cutoff collar branch already expresses local repair through exact splice or Petz/Fawzi--Renner recovery. What remains is to prove on natural OPH branches that these accepted recovery moves are repair-complete for overlap consistency, strictly \(\Phi\)-decreasing so that the Lyapunov termination theorem applies, locally confluent, and fully controlled on their declared domain, including the support/CPTP clause on the Petz branch.
