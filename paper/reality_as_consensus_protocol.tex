...
\begin{abstract}
This paper gives the consensus-theoretic core of Observer-Patch Holography (OPH). If physics is built from local observer descriptions that must agree on overlaps, the first question is whether different repair orders lead to different worlds. On the fixed-cutoff OPH collar branch, the local repair step is not left as an abstract rewrite primitive: it is read from exact Markov splice on exact collars or from declared Petz/Fawzi--Renner reconciliation on recoverable collars, and committed only under the declared local-fit contract on the overlaps it touches. That contract makes the OPH inconsistency potential \(\Phi\) a Lyapunov functional for the accepted repair moves, so the repair dynamics terminates; once repair completeness and local confluence hold, it has a unique schedule-independent normal form. The paper also identifies the loop obstructions to global consistency, shows why physical uniqueness belongs on the gauge quotient rather than on raw microscopic representatives, and gives a fixed-point account of stable classical records. This is the finite patch-net theorem package that supports the broader relativity, gauge, particle, and observer branches in the OPH suite.
\end{abstract}
...
moves, so termination is derived on the finite patch net; once repair completeness and local
confluence hold on that declared branch, every initial configuration has a unique
schedule-independent normal form. The second layer concerns obstructions: pairwise overlap agreement does not ensure a
...
\begin{enumerate}
\item \textbf{Asynchronous confluence.} For the declared accepted repair law, the local-fit contract makes \(\Phi\) a Lyapunov functional and hence gives termination; under repair completeness and local confluence, every initial state has a unique normal form, independent of update schedule (Theorem~\ref{thm:confluence}).
...
The fixed-point theorem below isolates the separate remaining burdens cleanly: the declared
repair law includes the touched-overlap local-fit contract that yields Lyapunov descent of
\(\Phi\) on accepted moves, so the remaining burdens are repair completeness, local
confluence, and, on the Petz branch, the support/CPTP clause stated later in
...
local-fit contract of Definition~\ref{def:localfit}. What the present theorem package assumes is
repair completeness and local confluence. On the Petz branch, full
CPTP action on all inputs also requires the support clause recorded later in
Proposition~\ref{prop:petz-cptp}.
\end{remark}
...
For the accepted repair law of Definitions~\ref{def:repair} and \ref{def:localfit}, under
Assumptions~\ref{ass:complete} and \ref{ass:confluence}, every initial state $s\in\Sigma$ has a unique normal form
\[
...
We prove confluence by well-founded induction on the finite ordered set \(\Phi(\Sigma)\). Take any
$s\in\Sigma$ and suppose $s\to^* t$ and $s\to^* u$. We must show $t$ and $u$ have a common descendant.

If one reduction has length~0, there is nothing to prove. Otherwise write $s\to s_1\to^* t$ and $s\to s_2\to^* u$. By local confluence, there exists $w$ with $s_1\to^* w$ and $s_2\to^* w$. Since $s\to s_1$ and $s\to s_2$, Proposition~\ref{prop:lyapunov-descent} gives $\Phi(s_1)<\Phi(s)$ and $\Phi(s_2)<\Phi(s)$. The induction hypothesis applies at $s_1$ and $s_2$. Hence $t$ and $w$ have a common descendant, and $u$ and $w$ have a common descendant. Therefore $t$ and $u$ have a common descendant.
...
problem here is to prove that the accepted
recovery-derived local moves satisfy repair completeness together with local confluence on the
declared branch, and the support/CPTP clause on the Petz branch. The touched-overlap local-fit
contract gives Lyapunov \(\Phi\)-descent on accepted moves. The point here is also that no
extra gauge-covariance axiom is needed once repair is formulated on the quotient.
...
This paper provides the finite patch-net foundation for the broader OPH suite. When the broader
...
\item \textbf{Recovery-channel control.} The fixed-cutoff collar branch expresses local repair through exact splice or Petz/Fawzi--Renner recovery and commits candidate moves only under the touched-overlap local-fit contract that yields Lyapunov descent of \(\Phi\). The remaining task is to prove on natural OPH branches that these accepted recovery moves are repair-complete for overlap consistency, locally confluent, and fully controlled on their declared domain, including the support/CPTP clause on the Petz branch and the preservation of the local-fit contract under refinement or branch changes.
...
written in exact-splice / Petz form; what remains conditional is the \textrm{CPTP}
