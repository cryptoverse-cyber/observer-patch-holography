...
\begin{abstract}
This paper gives the concrete microphysics side of Observer-Patch Holography. Patches, overlaps, records, and observers are written as explicit finite registers with declared readout, repair, and synchronization rules. The result is a fixed-cutoff architecture that can be simulated directly and used to analyze measurement, stable records, checkpoint/restoration, and observer synchronization inside one shared quantum system. The same setup also gives the edge-sector heat-kernel law that feeds the gauge-coupling sector. On the quantitative branch carried by the companion papers, the total/bulk/edge hierarchy has one exact self-similar balance point at the golden ratio, while an observer-supporting universe requires a small detuning away from that exact symmetry. The paper does not claim a final UV completion. It gives the finite reference surface on which the rest of the program is built.
\end{abstract}
