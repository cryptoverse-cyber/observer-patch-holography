...
For the scope-cleaned OPH claim set, the recovered core is the D1--D5 relativity chain plus the D7--D9 Standard Model structural chain. D6 is a separate input-dependent capacity corollary, and the D10 gauge/electroweak closure together with the D11 Higgs/top branch are secondary, supplement-backed parts of the current implementation. The items below are therefore \emph{not} predictions of the recovered core, even when they are discussed elsewhere in the manuscript.

\textbf{Not predicted by the recovered core.}
\begin{itemize}
\item \textbf{Flavor data}: Yukawa matrices, flavor hierarchies, Koide structure, charged-lepton fits, and quark-texture exponents. These require extra ans\"atze such as a uniform $\mathbb Z_6$ center-label ensemble, circulant generation-space structure, or discrete exponent assignments.
...
\item \textbf{String/worldsheet reorganization}: the Gross-Taylor-type interpretation requires a controlled large-$N_{\mathrm{edge}}$ regime that is not part of the proved compact-gauge chain.
\item \textbf{Strong CP}: the manuscript does not derive $\theta_{\mathrm{QCD}}$; any obstruction-based resolution remains a future program rather than a recovered result.
\end{itemize}
...
The reading rule is therefore simple. Failure of a deferred flavor, dark-sector, baryogenesis, spectroscopy, or string continuation retracts that continuation only. Failure of D10 or D11 challenges the current numerical implementation. Only a failure in the recovered core would overturn the compact paper's Phase-I claim set.
