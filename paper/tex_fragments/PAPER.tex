...
\begin{itemize}
\item \textbf{Flavor and Yukawa data}: Yukawa matrices, flavor hierarchies, charged-lepton/Koide fits, quark-texture exponents, CKM/PMNS closure, and related downstream flavor fits. These require extra ans\"atze such as a uniform \(\mathbb Z_6\) center-label ensemble, circulant generation-space structure, discrete exponent assignments, or further external numerical machinery. The light-quark split remains a D12 continuation frontier rather than a recovered-core selector, the physical quark branch waits on a finite same-label left-handed orbit together with the provider/evaluator that exposes it, the charged-lepton lane fixes only the centered hierarchy until the uncentered determinant-line route is completed, and the neutrino continuation branch reaches the physical PMNS/hierarchy pattern while leaving the absolute spectrum one bridge invariant and one positive normalization scalar short of closure.
