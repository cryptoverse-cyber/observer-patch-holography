...
This subsection now advances one step farther than the fixed-cutoff weak-tail-generator boundary. At fixed cutoff one can transfer cut-center data to regulated null strips, obtain exact or controlled strip additivity on one inherited strip model, and derive a weak tail generator for the renormalized half-line family. On the OPH geometric scaling branch of Theorem 4.2, the null half-line blow-up net then inherits geometric dilation and therefore half-sided modular inclusion, so Borchers--Wiesbrock supplies a genuine positive translation generator without leaving that step as an extra physical premise. What is still not proved here is the downstream density upgrade and the relativistic null-stress identification. Accordingly the c.11-local bridge now consists of:
\begin{itemize}
\tightlist
\item
...
\end{itemize}
The later density formula and \(T_{kk}\) bridge are kept below only as explicit downstream templates rather than as fixed-cutoff theorems proved in this subsection.
...
\textbf{Downstream c.11--c.13 boundary.} The fixed-cutoff bridge established above is the end of the present subsection's proved content.
...
\textbf{Lemma 5.2f (Borchers promotion to a genuine null translation generator).} For the derived half-sided modular pair of Corollary 5.2e, Borchers--Wiesbrock yields a strongly continuous one-parameter unitary group \(U_\Omega(a)=e^{iaP_\Omega}\) with \(P_\Omega\ge0\) such that for \(a,b\ge0\),
\[
...
\]
The weak tail generator from Proposition 5.2c is therefore upgraded from a domain-level endpoint derivative to a genuine positive translation generator.

\textbf{Proof.} Apply the Borchers--Wiesbrock theorem to the half-sided modular inclusion \(\mathcal M_a(\Omega)\subset \mathcal M_0(\Omega)\) supplied by Corollary 5.2e. This gives the unique positive self-adjoint generator \(P_\Omega\) and the affine covariance relation with the modular group. Differentiating that covariance at \(a=0\) gives \([K,P_\Omega]=i2\pi P_\Omega\). QED.

\textbf{Theorem 5.2g (Downstream null-stress identification template).} If one adds the density-upgrade hypotheses of Lemma 5.2d and the relativistic scaling-limit null-stress premise, then on the half-line blow-up net whose half-sided modular inclusion is derived in Corollary 5.2e and whose positive translation group is given by Lemma 5.2f, one expects an operator-valued distribution \(T_{kk}(v,\Omega)\) with
\[
...
\item
  Borchers--Wiesbrock therefore supplies a genuine positive null-translation generator inside the bridge itself;
\item
...
\end{itemize}
This is the disciplined route from null-strip factorization to the later Jacobson branch without pretending that c.11 has already completed the remaining density-upgrade and null-stress steps.
...
\]
Here ``supplement-backed implementation'' is literal: the compact appendix records the core one-loop RG system, but the synchronized D11 outputs above also use the shared matching, low-scale extraction, and pole-mass conventions documented in the spectrum supplement. The appendix equations alone are not meant as a standalone reproduction recipe for these displayed numbers.
