...
\begin{quote}
OPH is an observer-centric reconstruction program for fundamental physics. From five canonical axioms, two external continuous configuration inputs in the current quantitative implementation, and an explicit theorem-local regulator/scaling-limit package, OPH develops a conditional Jacobson-type Einstein branch, conditional compact gauge reconstruction in the bosonic edge-sector branch, and quantitative branches whose status is separated into supplement-backed calibration checks, a Higgs/top critical-surface branch that adds no further continuous fit once the gauge trajectory and scale-setting branch are fixed, and phenomenological continuations. The Standard Model and general relativity are treated as effective sectors of that underlying structure, while worldsheet/string reorganization is retained only as a continuation-level large-\(N_{\mathrm{edge}}\) branch.
\end{quote}
...
\textbf{Key conditionality.} The Lorentz/null-modular/Einstein package is a scaling-limit statement rather than a literal fixed-cutoff claim. Section 5 now removes any separate propagation, endpoint-control, or half-sided modular-inclusion premise from the fixed-cutoff null bridge and closes that bridge through the derived positive null-translation stage; the remaining explicit burdens are the density upgrade and the relativistic null-stress identification. The gauge group reconstruction yields \emph{a} compact group from a refinement-stable sector colimit; on the realized MAR-admissible branch the explicit structural-economy selector MAR (Minimal Admissible Realization) fixes the SM gauge quotient, the CP/UV admissibility window and MAR give \(N_g = 3\), and Witten's \(\mathrm{SU}(2)\) anomaly parity with that realized generation count gives \(N_c = 3\) once the admissible class includes one connected abelian charge factor (see §6.2 and Part II of this manuscript).
...
\end{itemize}
This is the disciplined route from null-strip factorization to the later Jacobson branch without pretending that the null modular bridge has already completed the remaining density-upgrade and null-stress steps.
...
\textbf{Experimental status}: PDG compiles upper limits of order \(10^{-18}\) eV (approximately \(10^{-27}\) GeV). Current bounds are consistent with the exact-zero consequence to present experimental sensitivity.
...
\textbf{Experimental status}: PDG 2025 lists m\_g ≤ 1.76 × 10⁻²³ eV/c² (90\% credibility) from gravitational wave dispersion analysis. The GW speed bound from GW170817 constrains (c\_GW − c)/c to \textasciitilde10⁻¹⁵. Current bounds are consistent with the exact-zero consequence to present experimental sensitivity.
...
Current searches are fully consistent with these structural consequences.

These are exact symmetry- or quotient-protected outputs: the empirical content is that violations remain excluded up to present experimental sensitivity.
...
\subsubsection{6.14a IBM Quantum Cloud hardware benchmarks}\label{614a-ibm-quantum-cloud-hardware-benchmarks}

The lattice calculations above are internal numerical validations. A first IBM Quantum Cloud campaign now adds an external hardware check of whether OPH-motivated reduced-sector structures survive preparation and readout on real superconducting-qubit devices. These runs are still small-sector consistency benchmarks rather than a standalone confirmation of the full framework, but they matter because they probe the same recoverability and heat-kernel observables on physical hardware rather than only in simulation.
...
\item
  \textbf{Z₃ hardware sanity check.} The overconstrained \(Z_3\) extraction passes cleanly on hardware: across prepared \(t = 0.30, 0.60, 0.90\), the mean extracted \(t\) remains within about \(0.02\) of the target, the two independent extractions agree to within \(0.0142\), \(0.0034\), and \(0.0043\), and leakage stays below \(0.1\%\). This shows that the reduced-sector preparation and readout path is internally coherent on-device before sharper ratio claims are interpreted.
\item
  \textbf{Z₅ exact-ratio test.} The parameter-free OPH target is \(\Delta_2/\Delta_1 = \varphi^2 \approx 2.618\). Across repeated sweeps on \texttt{ibm\_marrakesh} and one replication on \texttt{ibm\_fez}, the best \texttt{ibm\_marrakesh} points land within \(0.8\%\), \(1.2\%\), and \(1.5\%\) of the target, while a focused high-shot rerun at \(t = 0.90\) lands within \(2.6\%\). The \texttt{ibm\_fez} replication is noisier but remains in the same neighborhood rather than producing a clean contradictory ratio.
\item
  \textbf{S₃ nonabelian ratio test.} The first nonabelian hardware run revealed a real layout-dependent bias. After reversing the qubit layout, the mitigated ratio moves from \(1.8724\) to \(2.0299\) against the OPH target \(2.0000\), and a direct confirmation run returns \(2.0657\). This is important because the nonabelian target reappears once the hardware mapping is corrected, indicating an identifiable device/layout effect rather than a structural failure of the OPH prediction.
\end{itemize}

Taken together, these IBM runs are best read as early hardware evidence for the local OPH edge-sector picture. They show that the OPH-favored recoverability ordering survives on two real backends, the abelian exact-ratio tests land near the predicted parameter-free values, and the first nonabelian test returns to the expected ratio once a diagnosed layout bias is corrected. This does not yet validate the full framework, but it does move the core reduced-sector pattern from pure simulation into real-device experimental support.
...
The Z₆ quotient structure provides a possible phenomenological route to the fermion mass hierarchy once one adds the uniform center-label ansatz and the defect-suppression hypothesis, without introducing continuous Yukawa parameters.
