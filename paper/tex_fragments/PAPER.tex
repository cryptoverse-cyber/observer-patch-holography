...
\item
  \textbf{Microscopic theory}: Quantum link models (Section 2.6) provide one explicit UV realization of R0/R1 and give EC + Markov collars automatically; they are examples, not proofs of a unique microscopic completion. What remains: (i) a microscopic derivation of Recoverable Generalized Entropy, and (ii) ensuring modular flow becomes geometric in the continuum limit (the modular-covariance / GB gap). The latter likely requires a holographic or relativistic regime, which is not automatic in generic lattice gauge systems.
\item
  \textbf{Loop gluing beyond central defect}: At fixed cutoff the remaining named obstruction is the genuinely noncentral 2-group defect; quantitative matching to EFT anomalies remains open.
\end{itemize}
...
\begin{itemize}
\item \textbf{Flavor and Yukawa data}: Yukawa matrices, flavor hierarchies, charged-lepton/Koide fits, quark-texture exponents, and downstream hadronic or flavor fits. These require extra ans\"atze such as a uniform \(\mathbb Z_6\) center-label ensemble, circulant generation-space structure, discrete exponent assignments, or further external numerical machinery. Current local particle audits sharpen that boundary rather than weaken it: the light-quark split remains a D12 continuation frontier rather than a recovered-core selector.
...
\item \textbf{Calibration sector}: the D10 numerical implementation depends on supplement-backed running, matching, and threshold conventions. On the current local implementation the present carrier closes its own exact \(W/Z\) chart, but exact PDG \(W/Z\) still requires a repair branch beyond that carrier.
\end{itemize}

The strongest current local descendant downstream of the D10 gauge core is a compact Higgs/top forward seed whose emitted forward path closes on its own local certificate. That result remains a secondary quantitative descendant rather than a recovered-core theorem or a completed particle-spectrum closure.
