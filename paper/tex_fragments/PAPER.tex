...
\bottomrule\noalign{}
\endlastfoot
\textbf{D1} & Theorem 3.1: schedule-independent normal-form statement for the declared recovery-derived repair relation & overlap-consistency setup on finite patch nets together with the declared fixed-cutoff collar recovery step & finite-valued Lyapunov descent on the inconsistency potential; Newman's lemma & finite patch net, repair completeness, strict \(\Phi\)-descent, and local confluence on the declared recovery branch; stated Petz support/CPTP control where that branch is used & structural theorem \\
...
the accepted recovery-derived moves are repair-complete for overlap consistency, strictly
\(\Phi\)-decreasing so that \(\Phi\) is the finite-patch Lyapunov functional, locally confluent,
and controlled on the declared Petz domain.
