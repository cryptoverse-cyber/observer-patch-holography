...
\textbf{Historical shorthand G / A5 (cap-modular tangent-limit package):} The current compact-note ledger no longer treats cap isotropy or Euclidean regularity as separate free premises. What is needed for the BW\(_{S^2}\) theorem is a controlled refinement/scaling family whose rescaled collar neighborhoods converge to an \(\mathrm{SO}(2)\)-invariant tangent fixed point of the same local MaxEnt branch, together with the standard half-space Bisognano--Wichmann modular geometry on that tangent fixed point. Earlier references to Assumption G and premise A5 should be read as names for pieces of this single tangent-limit package.
...
\textbf{D3} & Theorems 4.2--4.3: geometric modular flow and Lorentz branch & Canonical 1--4 & Bisognano--Wichmann modular geometry; conformal-group classification & controlled refinement limit together with the tangent fixed-point / tangent-half-space comparison used in Theorem 4.2 & conditional scaling-limit theorem \\
...
\textbf{Theorem 4.2 (\(\mathrm{BW}_{S^2}\) from collar locality and tangent-half-space control).} Assume: (i) the collar refinement limit (Assumption F) together with vanishing exact-Markov modulus and transported modular remainder on the chosen collar family, (ii) MaxEnt selection with rotationally invariant constraints (Assumptions B-C), and (iii) after rescaling by the collar width near any smooth point of \(\partial C\), the collar modular flow converges on local observables to the modular flow of the corresponding tangent half-space fixed point of the same MaxEnt branch, with standard Bisognano--Wichmann normalization on that tangent half-space. Then in the controlled refinement/scaling limit, for each cap \(C\), modular flow is the unique conformal dilation that preserves \(C\) and fixes its boundary circle, with KMS normalization \(\beta = 2\pi\). Equivalently,

\[
K_C = 2 \pi B_C.
\]

\textbf{Proof.} Corollary 4.1c upgrades the controlled collar family to exact collar modular identities in the scaling limit, so the modular generator localizes to an arbitrarily thin collar of \(\partial C\). Rotational invariance around the boundary then forces the limiting collar generator to commute with the \(\mathrm{SO}(2)\) stabilizer of the boundary circle. Among conformal transformations of \(S^2\), the connected noncompact 1-parameter subgroup with that property is the cap-preserving dilation, so \(K_C=\beta_C B_C\) for some constant \(\beta_C>0\). Blowing up a smooth boundary point by the collar scale identifies the rescaled modular flow with the tangent half-space modular flow from assumption (iii). The tangent Bisognano--Wichmann theorem fixes that half-space generator to have normalization \(2\pi\), so the same blow-up forces \(\beta_C=2\pi\). Hence \(K_C=2\pi B_C\). QED.
...
\textbf{Proof.} Once Theorem 4.2 identifies cap modular flow with the cap-preserving conformal dilation, the screen kinematics generated by cap modular flow and rotations lies in the orientation-preserving conformal group of \(S^2\). That group is the M\"{o}bius group \(\mathrm{PSL}(2,\mathbb{C})\), which is isomorphic to the connected Lorentz group. QED.
...
We record an internal route toward the stress tensor that avoids assuming a UV CFT on small caps. Much of the EFT bridge can be tied to the same Markov structure (A4), geometric modular flow (BW\(_{S^2}\)), and explicit scaling-limit regularity premises used elsewhere in the argument. The crucial bookkeeping point is the same as in Section 4.1: exact null-strip additivity is justified only for exact Markov strip states, or for a controlled fixed-cutoff strip family whose distance to the exact Markov set goes to zero. Small strip CMI is not silently upgraded to exact identities.

\textbf{Theorem ladder summary.} The stress-tensor package is organized as a chain with explicit hypotheses; where the synchronized ledger keeps a step as premise rather than theorem, that status is left explicit:

\textbf{Read-along chain:}
\begin{itemize}
\tightlist
\item
  R0, R1 \(\Longrightarrow\) null-EC (Proposition 5.2a)
\item
  null-EC + exact Markovity or controlled exact-Markov strip limit \(\Longrightarrow\) N1 additivity (Corollary 5.2b)
\item
  B + MX \(\Longrightarrow\) bounded-interval endpoint control (Proposition 5.2c)
\item
  bounded-interval endpoint control + half-line differentiability \(\Longrightarrow\) density formula (Lemma 5.2d)
\item
  N2 is kept explicit as the half-sided modular-inclusion premise in the scaling regime
\item
  N2 + density + relativistic null-stress identification \(\Longrightarrow\) \(T_{kk}\) bridge with explicit carried error (Theorem 5.2g below)
\end{itemize}
...
\begin{quote}
\textbf{Proposition 5.2a (Null-EC from R0, R1).} Under the regulator premises R0 and R1 (type-I at UV plus boundary gauge invariants) applied to cuts at \(v=v_1,v_2\), the buffer strip algebra \(\mathcal A(J)\) has a central decomposition into edge labels at the two cuts:
\[
\mathcal H_J
\cong
\bigoplus_{\alpha_1,\alpha_2}
\left(
\mathcal H_{j_L^{\alpha_1,\alpha_2}}
\otimes
\mathcal H_{j_R^{\alpha_1,\alpha_2}}
\right),
\]
with center generated by projectors \(P_{\alpha_1,\alpha_2}\). Within each block, \(\mathcal A(I_-\cup J)\) acts only on the left factor and \(\mathcal A(J\cup I_+)\) acts only on the right factor.

\textbf{Proof.} A cut at fixed \(v\) is a boundary for the strip region in the same kinematic sense in which \(\partial C\) is a boundary for a spatial collar. The EC proof of Section 2.3 is therefore unchanged: type-I factorization at the regulator and boundary gauge invariance imply a Schur-lemma decomposition with center labels at the cuts. QED.
\end{quote}

\begin{quote}
\textbf{Corollary 5.2b (Controlled N1 from null-strip EC).} Let \((I_-):(J):(I_+)\) be a fixed regulated strip tripartition and define
\[
\mathfrak M_{I_-:J:I_+}
:=
\left\{
...
\right\},
\]
together with the strip exact-Markov modulus
\[
...
\right\}.
\]
On this fixed finite-dimensional strip model,
\[
\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon)\to 0
\qquad
(\varepsilon\downarrow0).
\]

If the strip reference state is exact Markov, then
\[
K_{I_-\cup J\cup I_+}
=
K_{I_-\cup J}+K_{J\cup I_+}-K_J
\]
up to a central boundary term.
...
\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon).
\]
Then every bounded observable on \(I_-\cup J\) or \(J\cup I_+\) differs from the exact-Markov reference by at most \(O(\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon))\). If the relevant strip marginals are uniformly faithful with lower spectral bound \(\lambda_\ast>0\), then
\[
...
\]
Independently, Fawzi--Renner gives a recovered comparison state with trace-norm error \(r_{\mathrm{FR}}(\varepsilon)=2\sqrt{1-e^{-\varepsilon}}\le 2\sqrt{\varepsilon}\). Thus exact N1 is available only at exact Markovity or in a controlled strip family with \(\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon_J)\to0\).
\end{quote}
...
\begin{quote}
\textbf{Corollary 5.2e (Heuristic blow-up motivation for N2).} In the synchronized dependency ledger, N2 (half-sided modular inclusion) remains an explicit scaling-limit premise. The BW\(_{S^2}\) blow-up discussion below only motivates why one might expect that premise in a geometric modular phase.
...
\]
For \(t\ge 0\), the half-line \(v>v_0\) maps into itself, which is the geometric pattern behind the half-sided modular-inclusion premise.
\end{quote}
...
\item consecutive null strips satisfy the null-EC structure above, with strip CMI controlled by \(\varepsilon_I\);
...
\eta_I^{\mathrm{EFT}}
\]
collects the constructive recoverability error, the exact-Markov replacement error when modular Hamiltonians are compared on a fixed faithful strip model, and the separate relativistic scaling-limit remainder. For domain vectors \(\psi,\phi\),
\[
\bigl|
...
\textbf{Proof.} Controlled null modular additivity comes from Corollary 5.2b. Lemma 5.2d gives the half-line endpoint-differentiation formula. Corollary 5.2e and Lemma 5.2f identify the resulting half-line density with the positive generator of null translations. The relativistic scaling-limit premise upgrades that density to \(T_{kk}\), while the three remainder sources are collected in \(E^{(\eta)}\). The null-to-tensor upgrade is the preceding lemma. QED.

\textbf{Gap-closure status.} The null bridge is therefore conditional rather than automatic, but the exact or controlled boundary is now explicit:
\begin{itemize}
\tightlist
\item
  \textbf{N1} is exact for exact Markov strip states, and otherwise is replaced by a controlled limit with carried errors \(r_{\mathrm{FR}}(\varepsilon_I)\) and \(\delta^{\mathrm M}_{I}(\varepsilon_I)\).
\item
  \textbf{N2} remains an explicit scaling-limit premise; Corollary 5.2e is motivation, not a theorem upgrade.
\item
...
\end{itemize}
This is the disciplined route from small strip CMI to the exact null-splice identities later used in the Jacobson branch.
...
\textbf{The \(2\pi\) KMS normalization.} From Theorem 4.2, the modular flow around a smooth cut inherits the tangent half-space Bisognano--Wichmann normalization \(2\pi\) through the controlled collar blow-up. This is the same rigidity that fixes Unruh/Hawking temperature normalization. The period is determined by the axioms together with the stated scaling-limit package.
...
\textbf{Normalization from modular geometry.} The BW\_\{S²\} theorem gives modular flow on a cap as the conformal dilation preserving the cap and fixing its boundary, with KMS normalization β = 2π inherited from the tangent half-space Bisognano--Wichmann comparison. Near an entangling surface, modular flow is a boost; in Euclidean continuation it becomes an angular coordinate with period 2π. The dilation parameter is s ∼ ln(μ/μ₀).
...
\textbf{D3} & Theorems 4.2--4.3 Lorentz branch & Bisognano--Wichmann modular geometry; conformal-group classification & controlled refinement limit together with the tangent fixed-point / tangent-half-space comparison used in Theorem 4.2 & Phase I conditional scaling-limit theorem \\
\textbf{D4} & Section 5.2 null modular bridge & Borchers--Wiesbrock; distribution theory for half-line differentiation & null-strip control, half-sided inclusion, and the relativistic null-stress identification premise; the endpoint/propagation controls are internal to the local finite-constraint MaxEnt branch & Phase I conditional bridge theorem \\
