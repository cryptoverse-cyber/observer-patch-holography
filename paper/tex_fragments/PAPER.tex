...
\subsubsection{1.6 Summary: Premise ledger and compatibility map}\label{16-summary-complete-axiom-set}

For reference, the manuscript has one authoritative premise ledger: the five canonical axioms above, the two external inputs \(P\) and \(N_{\mathrm{scr}}\) when a quantitative branch uses them, and the theorem-local technical premises declared in Section 1.4. Historical labels are retained below only as a compatibility map for later sections.

{\def\LTcaptype{none}
\begin{longtable}[]{@{}>{\RaggedRight\arraybackslash}p{0.18\textwidth}>{\RaggedRight\arraybackslash}p{0.34\textwidth}>{\RaggedRight\arraybackslash}p{0.22\textwidth}>{\RaggedRight\arraybackslash}p{0.18\textwidth}@{}}
\toprule\noalign{}
Item & Present meaning & Typical use & Status \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Canonical 1 & Screen Net & global net structure & axiom \\
Canonical 2 & Overlap Consistency & compatibility on overlaps & axiom \\
Canonical 3 & Local MaxEnt and Refinement Stability & state selection and RG stability & axiom \\
Canonical 4 & Recoverable Generalized Entropy & focusing plus recoverability structure & axiom \\
Canonical 5 & Minimal Admissible Realization (MAR) & selection on admissible gauge branches & axiom \\
\(P\) & pixel area \(a_{\mathrm{cell}}/\ell_P^2\) & quantitative particle-physics scale & external input \\
\(N_{\mathrm{scr}}\) & total screen capacity & cosmological-capacity branch & external input \\
\textbf{A1}, \textbf{A2} & Screen Net, Overlap Consistency & older proof shorthand & legacy shorthand \\
\textbf{B}, \textbf{I} & regulator-scale MaxEnt clause and refinement-stability clause of Canonical 3 & Sections 2, 4, 6 & theorem-local shorthand \\
\textbf{A3} & Recoverable Generalized Entropy & gravity/focusing discussions & legacy shorthand \\
\textbf{A4}, \textbf{F}, \textbf{MX} & recoverability and collar/null-strip control hypotheses used under Canonical 4 & Sections 2 and 5 & theorem-local shorthand / premise \\
\textbf{R0}, \textbf{R1} & type-I regulator and boundary gauge invariants & EC and gauge reconstruction & technical premise \\
\textbf{LR} & Lieb-Robinson locality at the UV scale & modular-locality control & technical premise \\
\textbf{D}, \textbf{E}, \([z]=0\) & gluing, central-defect, and transportability hypotheses & gauge branch & technical premise \\
\textbf{G} & Euclidean regularity & BW/KMS normalization & technical premise \\
\end{longtable}
}
...
With this ledger, every theorem has a declared premise list, every external result is cited, and the manuscript no longer treats the historical shorthand package as a competing axiom basis.
...
\subsubsection{8.1 Classification of results}\label{81-classification-of-results}

\textbf{Genuinely derived from axioms:}

\begin{itemize}
\tightlist
\item
  \textbf{Photon mass = 0}: Assumption D (gauge-as-gluing) → gauge invariance → no mass term
\item
  \textbf{Graviton mass = 0}: Entanglement equilibrium → diffeomorphism invariance → no mass term
\item
  \textbf{Gluon mass = 0}: Same as photon (gauge-as-gluing for SU(3))
\item
  \textbf{Lorentz group}: canonical OPH package + F/G/H technical premises → BW → Conf(S²) ≅ SO(3,1)
\item
  \textbf{CPT invariance}: Lorentz kinematics + locality → CPT theorem
\item
  \textbf{Charge conservation}: Unbroken U(1)\_em gauge symmetry
\item
  \textbf{Newton\textquotesingle s constant formula}: G = a\_cell / (4 \={ℓ}(t)) from edge entropy density (Section 5.4)
\item
  \textbf{Discrete area spectrum}: Log-integer area eigenvalues from edge sectors (Section 5.11); this remains stable
\item
  \textbf{Discrete Hawking/GW comb}: The specific comb pattern ΔE\_k = k\_B T\_H ln(k) follows if integer-multiplication transitions dominate; generic transitions would give a denser log-rational spectrum
\end{itemize}

\textbf{Derived given assumed matter content:}

\begin{itemize}
\tightlist
\item
  \textbf{Hypercharges (exact rationals)}: SM matter content assumed
\item
  \textbf{Charge quantization}: Z₆ quotient from realized spectrum
\item
  \textbf{Z₆ congruence rule}: SM global group structure
\item
  \textbf{Conditional Z₆ center-label entropy scale \(\approx \log_2 6\) bits}: uniform sixfold center-label ansatz on quotient sectors
\item
  \textbf{Yukawa hierarchy y\_f ∝ 6\^{}\{-n\_f\}}: Z₆ defect suppression with integer charges
\end{itemize}

\textbf{Precision validations against existing data:}

\begin{itemize}
\tightlist
\item
  \textbf{Strong coupling}: α\_s(M\_Z) ≈ 0.1175 vs PDG 0.1177 ± 0.0009 (within 1σ)
\item
  \textbf{Weak mixing angle}: sin²θ\_W ≈ 0.2311 vs PDG \^{s}²\_Z = 0.23122 ± 0.00006 (MS-bar, \textasciitilde2σ)
\item
  \textbf{Z₆ charge quantization}: PDG bounds confirm \textbar q\_p + q\_e\textbar/e \textless{} 10⁻²¹, fractional charge abundance \textless{} 10⁻²²/nucleon, CMS excludes fractionally charged particles to 640 GeV (Section 6.12)
\item
  \textbf{Casimir log-gap ratios}: Lattice SU(3) data (Bali, hep-lat/0006022) confirms ratios 9/4, 5/2, 4, 9/2, 6 at percent-level precision (Section 8.1)
\item
  \textbf{Photon mass}: PDG bound m\_γ \textless{} 10⁻¹⁸ eV confirms exact zero
\item
  \textbf{Graviton mass}: PDG bound m\_g \textless{} 1.76 × 10⁻²³ eV confirms exact zero
\end{itemize}

\textbf{Derived under the extended gauge-selection package \(T_{\mathrm{ext}}\) (canonical five axioms + explicit transportability + gauge-reconstruction hypotheses):}

\begin{itemize}
\tightlist
\item
  \textbf{Product gauge group}: Derived from minimal coupled carrier \(\mathbb{C}^3 \otimes \mathbb{C}^2\) under MAR
\item
  \textbf{SM global gauge group \(SU(3) \times SU(2) \times U(1)/\mathbb{Z}_6\)}: MAR + admissibility (Part II of this manuscript)
\item
  \textbf{N\_c = 3}: Witten anomaly + MAR minimality (Theorem 6.14)
\item
  \textbf{N\_g = 3}: CP + asymptotic freedom + MAR minimality (Proposition 6.9)
\item
  \textbf{No gauge-mediated proton decay}: product group from MAR
\item
  \textbf{No magnetic monopoles}: Product group structure (no GUT-scale symmetry breaking)
\end{itemize}

\textbf{Consistency checks (not novel predictions):}

\begin{itemize}
\tightlist
\item
  \textbf{α\_s(M\_Z) ≈ 0.117 with MSSM spectrum}: MSSM GUT analyses (1990s)
\item
  \textbf{sin²θ\_W(M\_U) = 3/8}: Georgi and Glashow (1974)
\item
  \textbf{Witten anomaly constraint}: Witten (1982)
\item
  \textbf{GIM mechanism (no tree-level FCNC)}: Glashow, Iliopoulos, Maiani (1970)
\end{itemize}

The framework\textquotesingle s contribution to unification physics is: (1) a \emph{mechanism} for why couplings unify (geometric unification via shared edge diffusion), (2) a derivation of MSSM-like beta shifts from Z₆ quotient + Peter-Weyl structure (Sections 6.17, 6.23), achieving Δb ≈ (2.49, 4.17, 4.01) vs MSSM (2.5, 4.17, 4.0) with \textless1\% error, and (3) a product gauge group that forbids proton decay.

\textbf{Sharpest near-term precision target: Casimir log-gap ratios.}
...
Δ\_R₁ / Δ\_R₂ = C₂(R₁) / C₂(R₂) (exact, parameter-free)

where Δ\_R = ln(p₀/d₀) − ln(p\_R/d\_R) = t C₂(R).

The headline SU(3) prediction is:

\textbf{Δ₈/Δ₃ = 9/4 = 2.25} (adjoint/fundamental ratio)

This is the nonabelian analog of the Z₅ golden-ratio-squared test (φ² ≈ 2.618), which has already been validated to 0.04\% precision. The SU(3) ratio directly stress-tests the framework\textquotesingle s core claim that gauge couplings are encoded in edge-sector probabilities via a Laplacian/Casimir heat kernel.
...
So 2.25 is not a "generic" number one stumbles into. Checking this ratio to 10⁻³ relative accuracy in lattice SU(3) edge-sector measurements would provide strong evidence that the heat-kernel mechanism operates as predicted.

\textbf{Validation against lattice QCD static potentials.}

The Casimir-scaling structure has been tested in lattice SU(3) gauge theory. Bali (\href{https://arxiv.org/abs/hep-lat/0006022}{hep-lat/0006022}) computed static potentials for multiple representations and reported continuum-extrapolated ratios. At \(r/r_0 = 0.73\):
...
Over the range 0.46 ≤ r/r₀ ≤ 1.84, the RMS deviations from Casimir scaling are: \textbf{8}: 1.35\%, \textbf{6}: 2.03\%, \textbf{15}: 2.49\%. Higher representations show larger scatter due to string-breaking effects and statistics, but the overall pattern confirms Casimir scaling at the percent level.

This is not a fit; the ratios 9/4, 5/2, 4, 9/2, 6 are exact predictions from the heat-kernel law with no adjustable parameters. The lattice data validates the mechanism to the precision achievable with current methods.

\textbf{Gravity-sector precision ceiling.}

The gravity predictions are symmetry-protected exact zeros that experiments have pushed to extraordinary precision:

\begin{itemize}
\tightlist
\item
  \textbf{(c\_GW − c)/c}: Model predicts = 0 exactly. Bound: {[}−3×10⁻¹⁵, +7×10⁻¹⁶{]}
\item
  \textbf{Graviton mass}: Model predicts = 0 exactly. Bound: ≤ 1.76×10⁻²³ eV/c²
\item
  \textbf{Dipolar radiation}: Model predicts none. Bound: δ\^{p}₋₂ ∈ {[}−4×10⁻⁶, 2×10⁻⁵{]}
\item
  \textbf{GW polarizations}: Model predicts tensor only. Pure non-tensor disfavored
\end{itemize}

These bounds already nail the exact-zero predictions to 10⁻¹⁵ fractional accuracy. The framework provides internal error control: matching this precision requires I(A:C\textbar B) ≲ 10⁻³¹, which is achievable via the exponential MX decay with δ/ξ \textasciitilde{} a few hundred.
