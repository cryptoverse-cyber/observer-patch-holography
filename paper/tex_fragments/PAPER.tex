...
After Section 2.3, the regulator package itself is no longer the open problem. The remaining gap is phase selection: the refinement-stable MaxEnt fixed point must lie in a regime whose modular flow is geometric on caps and whose null limits support the later relativistic bridge. The \textbf{Conformal-Modular Fixed Point (CMFP)} package is one candidate route to that regime. It is not a theorem of the canonical five axioms; it is a sharpened statement of what the UV completion would still need to realize.
...
The price is that CMFP-3 remains a phase statement: the refinement-stable MaxEnt fixed point must lie in the geometric modular action class. That is a concrete condition on the UV completion rather than an abstract axiom. It carries only the later geometric/null-modular burden; it no longer needs to justify the fixed-cutoff type-I/fixed-point regulator bookkeeping, which Section 2.3 already internalized.
...
We record an internal route toward the stress tensor that avoids assuming a UV CFT on small caps. Much of the "EFT bridge" package can be tied to the same Markov structure (A4), geometric modular flow (BW\(_{S^2}\)), and explicit scaling-limit regularity premises used elsewhere in the argument. In the synchronized dependency ledger, however, the geometric modular action and the half-sided modular-inclusion step are still treated as explicit technical premises rather than as derived theorems. The discussion below should therefore be read as heuristic support or an alternative route, not as the authoritative claim path.
...
\item
  N2 kept explicit as the half-sided modular-inclusion premise in the scaling regime
\item
  N2 + density ⟹ T\_kk ⟹ Einstein (Thm 5.1)
\end{itemize}

Each step is discussed below with explicit hypotheses and status labels.
...
This does not remove N2 as an independent input in the authoritative ledger; it only indicates the kind of continuum behavior that would support the premise. QED.
\end{quote}
...
\textbf{Proof.} Decompose \(X_{ab} = Y_{ab} + \phi \, g_{ab}\) where \(Y\) is traceless. Since \(g_{kk} = g_{ab} k^a k^b = 0\) for null \(k\), the null contractions only see \(Y_{kk}\). In local inertial coordinates, write \(k = (1, \hat{n})\) with \(|\hat n| = 1\). Then
...
\qquad\text{for all }\hat n \in S^2,
\]
where \(Y_{ij}^{\mathrm{TF}}\) is the traceless spatial part. The \(\ell=2\) spherical harmonics are linearly independent, so \(Y_{ij}^{\mathrm{TF}}=0\). Hence \(Y_{ab}=0\) and therefore \(X_{ab}=\phi \, g_{ab}\). QED.
...
\item
  \textbf{N2 (half-sided modular inclusion)}: kept as an explicit scaling-limit premise in the synchronized ledger. Corollary 5.2e records only the BW\(_{S^2}\) blow-up motivation for why such a premise may hold in a geometric modular phase.
\item
