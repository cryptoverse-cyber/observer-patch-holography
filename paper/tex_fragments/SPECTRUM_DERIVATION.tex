...
\(N_g\) (generations) & 3 & \texttt{N\_g\_DEFAULT} & Three independent constraints narrow the window: (1) CP violation requires \(N_g \geq 3\) (CKM phases \((N_g-1)(N_g-2)/2 > 0\)). (2) Asymptotic freedom of SU(2) with one Higgs doublet requires \(N_g(N_c+1) < 43/2\), giving \(N_g \leq 5\). (3) MAR (fourth component of the complexity vector) picks the smallest value in \(\{3, 4, 5\}\). \textbf{Result}: \(N_g = 3\). & Proposition 6.9 (\S{}6.4); Part II of this manuscript \S{}7 \\
Gauge group & \((SU(3)\times SU(2)\times U(1))/\mathbb{Z}_6\) & --- & Tannaka-Krein / Doplicher-Roberts reconstruction from a refinement-stable edge-sector category yields a compact gauge group \(G\) (Theorem 6.1, \S{}6.1). The Selection Axiom MAR then selects the SM factors once the admissible class includes one connected abelian charge factor: the minimal coupled carrier \(\mathbb{C}^3 \otimes \mathbb{C}^2\) enforces the product structure, the commutant argument fixes the connected abelian factor to a single \(U(1)\), and the \(\mathbb{Z}_6\) quotient follows from hypercharge quantization (Proposition 6.6). & \S{}6.1--6.2; Part II of this manuscript \\
...
This part starts at the quantitative layer. D1--D5 together with D7--D9 form the recovered core; D10--D11 live here; D12 stays downstream. This section marks which numbers are calibration-sector checks, which belong to the secondary quantitative branch, and which remain phenomenological continuations.
...
\]
adds no new continuous fit, so its failure would challenge D11 while leaving the D10 calibration closure intact. It sits outside the recovered core D1--D5 plus D7--D9. Charged-lepton/Koide fits, texture exponents, neutrino estimates, proton-spin bookkeeping, and downstream hadron extractions use additional ans\"atze, capacity assumptions, or external lattice/QCD machinery. Those belong to D12 or to later downstream numerical layers and must not be read back into the core claim set.
...
The repository now ships that surface as a compact public export: a
