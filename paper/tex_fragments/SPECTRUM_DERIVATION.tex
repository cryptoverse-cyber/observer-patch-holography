...
The strongest non-calibration quantitative outputs are determined by structural content---boundary conditions, algebraic identities, topological data---that is \emph{logically independent} of the pixel constraint. Their dimensionless values are fixed before \(P\) enters the computation. Concretely:
...
\toprule
Output branch & Dimensionless form & What determines it (not \(P\)) \\
\midrule
\endhead
\bottomrule
\endfoot
\(m_e : m_\mu : m_\tau\) continuation & Koide ratios from \(Q = 2/3\), \(\delta = 2/9\) & Algebraic phase data plus an additional discrete-selection step for the exponent pattern; logically independent of \(P\) but weaker than the theorem-level branches. \\
...
\(m_f / m_t\) quark-texture continuation & \(\varepsilon^{n_f}\) with \(\varepsilon = 1/6\), integer \(n_f\) & \(\mathbb{Z}_6\) quotient topology + \(N_c\), \(N_g\), plus the texture ansatz used to assign exponents. \\
\end{longtable}
}
...
\textbf{Tier 2---Independent quantitative outputs.} These quantities are determined by structural content of OPH that is logically independent of the gauge-coupling calibration. They use \(P\) only to set an overall scale, and their specific values are not guaranteed by the calibration.

{\footnotesize
\begin{longtable}[]{@{}>{\RaggedRight\arraybackslash}p{0.30\textwidth}>{\RaggedRight\arraybackslash}p{0.62\textwidth}@{}}
\toprule
Quantity & Why it is an independent quantitative output \\
\midrule
\endhead
\bottomrule
\endfoot
...
\textbf{Tier 3---Phenomenological continuations and downstream numerical branches.} These outputs use additional ans\"atze, discrete scans, capacity estimates, or downstream numerical machinery beyond the core calibration and critical-surface branches.

{\footnotesize
\begin{longtable}[]{@{}>{\RaggedRight\arraybackslash}p{0.30\textwidth}>{\RaggedRight\arraybackslash}p{0.62\textwidth}@{}}
\toprule
Quantity & Why it is a phenomenological continuation / downstream branch \\
\midrule
\endhead
\bottomrule
\endfoot
\(m_e, m_\mu, m_\tau\) (charged leptons) & Uses the Koide phase data together with an additional discrete-selection step for the exponent pattern. Numerically sharp, but weaker in logical status than the critical-surface branch. \\
Quark mass hierarchy & Uses the \(\mathbb{Z}_6\) texture ansatz and captures hierarchy structure more reliably than individual masses. \\
\(m_{\nu_1}, m_{\nu_2}, m_{\nu_3}\) & Capacity-branch estimate from \(\log(\dim\mathcal{H})\); presently an order-of-magnitude branch rather than a benchmark quantitative mass formula. \\
Hadron masses & Downstream numerical continuation \(P \to \alpha_s \to \Lambda^{(3)} \to m_X\), with standard lattice-QCD systematics dominating the final errors. \\
\end{longtable}
}

\textbf{Tier 4---Parameter-free structural outputs.} These discrete quantities follow from the axioms and stated admissibility package with no continuous free parameter.
...
No gauge-mediated proton decay & Product gauge group (no leptoquark generators) \\
\end{longtable}
}

\subsubsection{1B.4 What OPH Achieves}\label{1b4-what-oph-achieves}

The Standard Model has 19 free parameters (or 26 with neutrino masses). OPH compresses the current implementation to one primary particle-physics calibration input (\(P\)) plus one cosmological/capacity input (\(\log \dim \mathcal{H}\)), with structural content such as the gauge group and generation count derived from axioms and admissibility.

The strongest independent quantitative content---Tier 2---is the Higgs/top critical-surface branch. The charged-lepton, texture, neutrino, and hadron sectors sit in Tier 3: they are useful continuations, and in the charged-lepton case numerically very sharp, but they do not currently have the same logical status as the critical-surface branch.
...
\(P\) is analogous to Newton's constant \(G\) in general relativity, or \(\alpha\) in QED: a dimensionful parameter that must be measured. The framework's claim is not ``we predict everything from nothing'' but rather: \textbf{from a small external input set and a set of axioms, the framework sharply constrains particle physics and organizes which outputs are structural, which are calibration-sector, which are independently quantitative, and which remain phenomenological continuations.}

The gauge couplings at \(m_Z\) are consistency checks. The Higgs and top masses are the main independent quantitative outputs. The charged-lepton, quark-texture, neutrino, and hadron branches are weaker continuations with explicitly stated limitations.
...
where \(g_Y(M_U)\) and \(g_2(M_U)\) are obtained by running the OPH-derived couplings from \(m_Z\) to \(M_U\) using 1-loop SM \(\beta\)-functions.

\subsubsection{5.2 RG Evolution to Low Scales}\label{52-rg-evolution-to-low-scales}

The coupled system \((y_t, \lambda, g_Y, g_2, g_3)\) is integrated from \(M_U\) down to \(\mu \sim m_t\) using RK4 with 1-loop SM \(\beta\)-functions. The gauge couplings run analytically:
...
Starting from the current external input set --- the pixel area \(P = 1.63094\) and the screen capacity \(\log(\dim\mathcal{H}) \sim 10^{122}\) --- the OPH derivation chain produces outputs in four epistemic tiers:
...
\textbf{Independent quantitative outputs} (structurally independent of calibration):
\begin{itemize}\tightlist
...
\textbf{Phenomenological continuations / downstream branches}:
\begin{itemize}\tightlist
\item Charged leptons: \(e\), \(\mu\), \(\tau\) at \textless{} 0.025\% from the Koide branch plus a discrete exponent-selection step
\item 6 quark masses at correct order of magnitude with hierarchy reproduced, but 16--73\% individual errors
\item 3 neutrino masses presently at order-of-magnitude level and consistent with current bounds
\item Hadron masses from \(\Lambda_{\overline{\text{MS}}}^{(3)}\) via lattice QCD (10--20\% systematics)
\end{itemize}

\textbf{Parameter-free structural outputs} (from axioms and admissibility):
\begin{itemize}\tightlist
...
\item No gauge-mediated proton decay, \(\varepsilon = 1/6\), \(\delta = 2/9\), \(Q = 2/3\)
\end{itemize}
...
\(P\) is the primary particle-physics calibration input. The first arrow is calibration; the critical-surface branch then gives the main independent quantitative outputs, while later flavor, neutrino, and hadron steps are weaker continuations. The framework's claim is not ``we predict everything from nothing'' but: \textbf{from a small external input set and structural axioms, the framework sharply constrains particle physics while keeping the logical status of each branch explicit.}
