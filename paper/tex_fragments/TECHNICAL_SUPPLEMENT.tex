...
\textbf{Theorem 2.2 (Bisognano-Wichmann on \(S^2\) from tangent-half-space control):} In the controlled refinement/scaling-limit EFT regime, assume the collar exact-Markov modulus and transported modular remainder vanish along the chosen collar family, the local MaxEnt branch is \(\mathrm{SO}(2)\)-invariant around the entangling circle, and the rescaled collar modular flow converges to the modular flow of the corresponding tangent half-space fixed point with standard Bisognano--Wichmann normalization. Then cap modular flow acts as the unique cap-preserving conformal dilation with period \(2\pi\): \[\sigma_t^\omega(A) = \Delta^{it} A \Delta^{-it}\]
...
\textbf{D3} & geometric modular flow and Lorentz branch & Bisognano--Wichmann modular geometry; conformal-group classification & controlled refinement limit together with the tangent fixed-point / tangent-half-space comparison of Theorem 2.2 & Phase I conditional scaling-limit theorem \\
