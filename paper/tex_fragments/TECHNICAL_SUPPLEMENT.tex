...
\textbf{Theorem 2.2 (Geometric modular flow on caps in the OPH geometric branch).} Assume the OPH axioms, the regulator premises R0--R1, local MaxEnt/refinement stability, the collar-control hypotheses used earlier in the supplement, and the branch condition that the refinement-stable cap net belongs to the geometric modular class for caps and their conformal images. Then the limiting modular flow of a cap \(C\subset S^2\) is the unique cap-preserving conformal dilation. If \(B_C\) denotes its generator, then
\[
K_C=2\pi B_C.
\]
...
\textbf{Proof.} The recoverability/collar analysis localizes the modular defect to the shrinking collar and allows replacement of the physical state by an exact Markov reference with explicit carried error. On the chosen refinement branch, the limiting modular action on \(\mathcal A(C)\) is geometric, so it acts by conformal maps preserving \(C\) and fixing \(\partial C\). The stabilizer of a round cap in \(\mathrm{Conf}^+(S^2)\) has a unique connected noncompact one-parameter subgroup; denote its generator by \(B_C\). Therefore the limiting modular Hamiltonian is \(K_C=\kappa B_C\) for some \(\kappa>0\). The modular flow is KMS by definition, and the geometric Euclidean continuation fixes the angular period to \(2\pi\); hence \(\kappa=2\pi\). The displayed remainder \(E_C^{(\delta)}\) is exactly the carried collar-replacement error, so it vanishes in the controlled refinement limit. QED.

\textbf{Scope note.} The supplement now states the phase boundary explicitly: the fixed-cutoff OPH collar technology does \emph{not} by itself prove that the theory must enter this geometric modular branch. What it does prove is that, once that branch is realized, both the cap generator and the \(2\pi\) normalization are fixed.
...
\textbf{D4} & null modular bridge to \(T_{kk}\) & Borchers--Wiesbrock; distributional differentiation on half-lines & null-strip control, half-sided inclusion, and the relativistic null-stress identification premise; the endpoint/propagation controls are internal to the local finite-constraint MaxEnt branch & Phase I conditional bridge theorem \\
