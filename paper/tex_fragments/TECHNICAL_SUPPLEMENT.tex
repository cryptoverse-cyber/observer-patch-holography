...
\]
for an induced map \(R_{\ell\to L}\) on that multiplier space. The phrase ``refinement-stable branch'' therefore means persistence along a trajectory or invariant subset of this finite-dimensional map. In particular, when the gauge branch later speaks of a refinement-stable directed colimit of sectors, the refinement-stable qualifier refers to persistence along this Axiom-3 branch itself; the remaining extra burdens are transportability, symmetric braiding, and the bosonic fiber-functor clauses.

\textbf{What remains external.} The present internalization claim stops at the fixed-cutoff local-Gibbs/Lieb--Robinson/refinement package. Quantitative exponential mixing or exact-Markov replacement, when invoked later, is separate state information tied to the recoverability side of Axiom 4 rather than to Axiom 3 alone. The genuinely external technical premises are therefore exactly the canonical list \(T1\)--\(T7\): the Bisognano--Wichmann geometric scaling branch and its scope clause, fixed-cap stationarity, the null-stress/density-upgrade burden, vanishing transport obstruction when global sector transportability is invoked, and the symmetric/bosonic/fiber-functor conditions used in gauge reconstruction. No separate regulator premise for quasi-local propagation or refinement stability survives once Axiom 3 is unpacked in this way.
...
The collar package proves fixed-cutoff localization of modular nonadditivity on the finite regulator net, but it does not by itself determine the continuum modular geometry. At each regulator stage the cap algebra is type I, so a literal density matrix and modular Hamiltonian exist. The Lorentz statement is instead about the scaling-limit cap pair. Nothing in local MaxEnt alone proves that the scaling limit remains type I or that it lands in the Bisognano--Wichmann branch. The theorem-scope must therefore be stated as one honest conditionality boundary: on the refinement branch singled out by the BW premise, the scaling-limit modular automorphism group is geometric; away from that branch, the supplement claims only the fixed-cutoff collar control.
...
\]
As in the main text, exact collar formulas are read either literally at exact Markovity or along a controlled family with
\[
\delta^{\mathrm M}_{A_\delta:B_\delta:D_\delta}(\varepsilon_\delta)\to0.
\]

\textbf{Theorem 2.2 (Geometric modular flow on the OPH Bisognano--Wichmann branch).} Assume the OPH axioms, the regulator premises \(R0\) and \(R1\), the local MaxEnt/refinement-stability package, the collar-control hypotheses used in the supplement, and the same branch boundary as \textbf{T1}+\textbf{T3} in the main text: the realized refinement-stable scaling limit for caps lies in the Bisognano--Wichmann geometric modular class. Let \((\mathcal A_\infty(C),\omega_\infty^C)\) denote the scaling-limit cap pair, let \(\lambda_C(s)\subset\Conf^+(S^2)\) be the standard cap-preserving conformal subgroup normalized by \(v\mapsto e^{-s}v\) under null blow-up, and let \(\alpha_{\lambda_C(s)}\) be the induced automorphism of the scaling-limit cap net. Then
\[
...
At regulator scale the cap algebra is type I, so
\[
K_C^{(\delta)}:=-\log\rho_C^{(\delta)}
\]
exists, and replacing its action by the geometric cap-dilation action incurs only the carried errors
\[
r_{\mathrm{FR}}(\varepsilon_\delta)
\qquad\text{and}\qquad
...
K_C=2\pi B_C.
\]
In the generic continuum case, where the scaling-limit cap algebra has left the regulator class, the automorphism identity above is the correct statement and the modular action is generally outer rather than inner.

\textbf{Proof.} The fixed-cutoff collar results localize the modular defect to the shrinking collar and carry its deviation from an exact Markov reference explicitly through \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(4\lambda_\ast^{-1}\delta^{\mathrm M}_{A_\delta:B_\delta:D_\delta}(\varepsilon_\delta)\). The BW branch condition identifies the scaling-limit modular automorphism group with the standard cap-preserving conformal subgroup up to normalization:
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(\kappa_C t)}
\]
for some \(\kappa_C>0\). The modular group is KMS by definition, and the null-blow-up normalization fixes \(\kappa_C=2\pi\). Thus
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(2\pi t)}.
\]
If the limit algebra remains type I, this automorphism statement can be represented by \(K_C=2\pi B_C\). Otherwise the automorphism statement is the full theorem content, and the action is outer. QED.

\textbf{Scope note.} This is the exact supplement-level claim boundary. The supplement proves geometric modular flow only on the BW scaling branch just stated. It does not prove that MaxEnt alone selects that branch, and it does not claim that a density matrix survives inside the continuum observer algebra.
...
\]
This is a D12 continuation ansatz, not a D9-derived output, and the current OPH package does not prove that a nonequilibrium baryogenesis history weights each relevant event by a fixed power of \(\varepsilon\).
...
Generic phases violate this \(\Rightarrow\) CP-odd overlap data are generically available in OPH. But this does not yet derive the CP source terms that would enter a kinetic or Boltzmann treatment. Turning this observation into a baryogenesis theorem still requires an effective description that links cocycle phases to defect production, diffusion, washout, and the final baryon asymmetry.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Generation Structure and Flavor Continuations}\label{12-generation-structure-and-yukawa-hierarchy}

This section separates one structural result from several continuation-level flavor constructions. The realized counting chain \(N_g = 3\) belongs to the proved D9 package and is not re-derived here. By contrast, the \(\mathbb Z_6\)-based Yukawa suppression rule, the integer texture assignments, the charged-lepton fit, the Koide ratio/phase identification, and the RK completion below all require extra flavor ans\"atze and are not part of the recovered core.

These flavor constructions require additional ingredients: a unique flavor measure forcing a uniform \(\mathbb Z_6\) ensemble, a derivation of the map from defect data to Yukawa suppression \(y\sim 6^{-n}\), the circulant generation-space ansatz, a selector for the final exponent assignments, and the scheme/running/matching control needed to turn these constructions into derived masses. A recovered-core nonzero light-quark pure-\(B\) selector is not derived, so the light-quark split remains continuation-level.
...
\(b\) & 0.024 & 2.08 & 2 \\
\(c\) & 0.0073 & 2.75 & 3 \\
\(s\) & 5.3\ensuremath{\times}10\textsuperscript{-}\textsuperscript{4} & 4.21 & 4 \\
\(d\) & 2.7\ensuremath{\times}10\textsuperscript{-}\textsuperscript{5} & 5.87 & 6 \\
\(u\) & 1.3\ensuremath{\times}10\textsuperscript{-}\textsuperscript{5} & 6.29 & 6 \\
\(\tau\) & 0.010 & 2.56 & 3 \\
\(\mu\) & 6.1\ensuremath{\times}10\textsuperscript{-}\textsuperscript{4} & 4.13 & 4 \\
\(e\) & 2.9\ensuremath{\times}10\textsuperscript{-}\textsuperscript{6} & 7.11 & 7 \\
\end{longtable}
}
...
Nothing in this section belongs to the recovered structural chain. The balanced Hermitian circulant, the mode-balance condition \(Q=2/3\), the phase choice \(\delta=2/9\), and any numerical charged-lepton value are Phase-III continuation inputs layered on top of the already fixed discrete data
\[
N_g=3,\qquad
N_c=3,\qquad
G_{\mathrm{phys}}=\frac{\SU(3)\times\SU(2)\times\U(1)}{\mathbb Z_6}.
\]
The exact theorem surface is narrower: same-carrier readback is closed, absolute scale is not, and the missing derived objects are first promotion of the latent charged sector-response candidate \(\widehat C_e^{\mathrm{cand}}\) via \texttt{oph\_generation\_bundle\_branch\_generator\_splitting}, and then one affine-covariant absolute anchor \(A_{\mathrm{ch}}\) on the charged common-shift quotient. The local corpus proves neither exact vanishing nor uniform quadratic smallness of the descended commutator after the central split, so \(\widehat C_e^{\mathrm{cand}}\) remains latent only. A minimal extension route is a centered Schur-type \(P\!\to\!Q\!\to\!P\) transfer theorem for the descended commutator after the central split; even if that route is internalized, the affine anchor \(A_{\mathrm{ch}}\) remains a separate object.
...
2.2\times10^{-6}.
\]
This numerical proximity is a continuation benchmark and not a recovered-core output.

\subsection{\texorpdfstring{\(Z_3\) holonomy continuation bridge}{Z3 holonomy continuation bridge}}\label{132-zux2083-holonomy-parameterization}

Consider the minimal Hermitian circulant on generation space
\[
\Phi=aI+bP+b^*P^2,
\]
with \(P\) the cyclic shift. Its ordered roots are
\[
...
\subsection{Balanced branch and compare-only continuation values}\label{133-why-q--23}

\[
Q=\frac23
\qquad\Longleftrightarrow\qquad
\frac{|b|}{a}=\frac{1}{\sqrt2}.
\]
The condition \(Q=2/3\) is therefore a balanced-branch condition internal to the circulant continuation ansatz. It is not derived from the structural OPH chain by itself.

If one further uses the continuation phase choice
\[
\delta_{\mathrm{cont}}
=
\frac{N_c+1}{2N_cN_g}
...
\frac{2}{9},
\]
then at the scale-free normalization \(a=1\) the ordered roots are
\[
...
\]
The residual norm against the charged references is
\[
\left\|E_{\log,\mathrm{centered}}^{\mathrm{cont}}
-
E_{\log,\mathrm{centered}}^{\mathrm{ref}}\right\|
\approx 2.13\times10^{-5},
\]
so the continuation bridge gives near-exact centered-log shape closure up to one common absolute shift.

\subsection{Exact same-carrier readback and absolute-scale obstruction}\label{134-the-koide-phase-from-oph}
...
\]
For any charged source pair \((\eta,\sigma)\) on that carrier, the centered charged logs are
\[
e_{\log,\mathrm{centered}}
=
...
\frac{(3-x_2)\sigma+\eta}{6}.
\]
Hence every positive absolute charged scale \(g_e\) produces
\[
...
\[
Y_e(c):=e^c\,\mathrm{diag}\!\bigl(e^{E_e^{\mathrm{centered}}}\bigr)
\]
has the same centered log vector, the same same-carrier invariants, and the same mass ratios. Only the absolute masses rescale:
\[
(m_e,m_\mu,m_\tau)\mapsto e^c(m_e,m_\mu,m_\tau).
\]
Hence the present charged chain determines only the quotient class
\[
E_e^{\mathrm{centered}}\in\mathbb R^3/\langle(1,1,1)\rangle,
\]
not the absolute charged scale \(g_e\).

The typed current-family shell records
\[
\mu_{e,\mathrm{seed}}^{\mathrm{abs}}=\log(0.9231656602589082),
\qquad
\mu_{e,\mathrm{cand}}^{\mathrm{abs}}
=
\mu_{e,\mathrm{seed}}^{\mathrm{abs}}-\gamma_{\min},
\qquad
g_e^{\mathrm{cand}}=0.6822819838027987.
\]
This is representation-consistent but not theorem-grade. The compare-only continuation bridge records the common scale
\[
g_e^\star=0.04577885783568762
\]
required for exact absolute masses on that branch, equivalently
\[
\Delta_e^{\mathrm{abs},\star}
=
\log\!\frac{0.9231656602589082}{0.04577885783568762}
=
3.003986333402356.
\]
Those values are compare-only back-solves. The missing theorem-grade object is one affine-covariant absolute anchor \(A_{\mathrm{ch}}\) on the charged common-shift quotient.
...
\textbf{D7} & compact gauge reconstruction in the bosonic branch & Doplicher--Roberts / Tannaka reconstruction & T2 and T5--T7; the refinement-stable qualifier is internal to the local finite-constraint MaxEnt/refinement branch & Phase I conditional structural theorem \\
