...
\textbf{Abstract.} We show that, once Tannaka/DR reconstruction is applied to the refinement-stable edge-sector colimit, the extended gauge-selection package \(T_{\mathrm{ext}}\) , meaning the canonical five axioms together with the explicit transportability premise \([z]=0\) and the gauge-reconstruction hypotheses of Part I , selects the Standard Model connected Lie gauge-sector factors, the number of colors, and the number of generations once the admissible class includes one connected abelian charge factor. The realized hypercharge lattice then fixes the global gauge quotient:
...
The derivation proceeds within the extended gauge-selection package \(T_{\mathrm{ext}}\): the canonical five-axiom package from Part I, the explicit loop-coherent gluing premise \([z]=0\), and the refinement-stable gauge-reconstruction hypotheses used in Theorem 6.1.
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Canonical 1--2} & Screen Net and Overlap Consistency & Part I of this manuscript §1.3 \\
\textbf{Canonical 3} & Local MaxEnt and Refinement Stability & Part I of this manuscript §1.3--§1.4 \\
\textbf{Canonical 4} & Recoverable Generalized Entropy & Part I of this manuscript §1.3 \\
\textbf{Canonical 5} & Minimal Admissible Realization & Part I of this manuscript §1.3 and this document §3 \\
\textbf{R0}, \textbf{R1} & Finite-dimensional regulator and boundary gauge structure & Part I of this manuscript §1.4, §2.5 \\
\textbf{\([z]=0\)} & Loop-coherent gluing / DHR transportability & Part I of this manuscript §6.6, Prop. 6.1a \\
\textbf{Theorem 6.1 hypotheses} & Rigid symmetric \(C^*\) colimit category with faithful fiber functor & Part I of this manuscript §6.1 \\
\end{longtable}
}

\subsubsection{1.2 What the Core Already Proves}\label{12-what-the-core-already-proves}

From the canonical OPH package together with the regulator premises R0 and R1, the following is established (Part I of this manuscript §6.1):
...
This proves \emph{existence} of a compact gauge group but does not identify \emph{which} compact group is realized. The present document determines the connected Lie realization selected by the extended admissibility package.
...
\textbf{Theorem (Standard Model closure under extended admissibility).} Under the extended gauge-selection package \(T_{\mathrm{ext}}\), together with the refinement-stable reconstruction hypotheses of Theorem 6.1:
...
\textbf{Proof.} The gauge group derivation (§5) already fixes the color factor as \(SU(3)\) , i.e., \(N_c = 3\) , through the minimal coupled-carrier argument (Lemma 5.2). We give here the independent confirmation from anomaly constraints + MAR.
...
With \(N_c = 3\) (from §6): \(4N_g < \frac{43}{2}\), so \(N_g \leq 5\).
...
\(+ \;\text{apply MAR on the admissible class}\) \(\xrightarrow{\text{Thm (this doc)}}\) \(G_{\mathrm{phys}} = SU(3) \times SU(2) \times U(1)/\mathbb{Z}_6\), \(N_c = 3\), \(N_g = 3\)
