...
At each cutoff one has a finite tensor category of transportable edge charges. The reconstruction uses the refinement-stable directed colimit \(\mathsf{Sect}_\infty\) of those categories. By Theorem 6.1 (Tannaka/DR reconstruction~\cite{tannaka38,krein49,dr89,dr90}), there exists a compact group \(G\), unique up to isomorphism, with \(\mathsf{Sect}_\infty \simeq \mathrm{Rep}(G)\).
