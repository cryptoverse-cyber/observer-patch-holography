...
\begin{abstract}
This paper is the compact statement of what Observer-Patch Holography (OPH) currently claims at
...
This section summarizes the claim tiers used throughout. The three-phase boundary is
\[
...
\section{Five Axioms, Two External Continuous Inputs, and Technical Premises}\label{sec:premise-ledger}

This section states the premise ledger and dependency map for the compact paper. The OPH basis used below consists of the five core axioms, the two external continuous inputs listed next, and the external technical premises \(T1\)--\(T7\) collected afterward. On the fixed-cutoff local finite-constraint MaxEnt branch, quasi-local propagation and refinement-stability control are internal consequences; Theorem~\ref{thm:bw} fixes the cap generator and its \(2\pi\) normalization on the Bisognano--Wichmann geometric branch; and Corollary~\ref{cor:n2} derives the null half-sided modular pair inside the null bridge. The remaining theorem-external items are the scaling-limit scope clause, the geometric-branch selection burden, the density-upgrade burden, fixed-cap stationarity, the bosonic/fiber-functor fork, the relativistic null-stress identification premise, and the vanishing obstruction premise where invoked.
...
The compact paper uses one canonical numbered list of external technical premises. Derived fixed-cutoff regulator bookkeeping, quasi-local propagation, endpoint control, and the fixed-cutoff half-sided modular pair are not listed here because they are already established internally from the axioms and earlier results. The seven external items are:
...
The collar analysis identifies where modular nonadditivity can live, but by itself it does not determine the continuum geometric action of the limiting modular group. The theorem therefore works on the refinement-stable cap branch lying in the Bisognano--Wichmann geometric modular class of \textbf{T1}. On that branch, OPH supplies a controlled reduction from the regulator collar to the refinement limit together with the \(2\pi\) normalization and carried-error statement for the standard cap-preserving conformal subgroup.
...
Step 2: passage to the Bisognano--Wichmann geometric branch. By the \textbf{T1} branch hypothesis, the refinement-stable cap net belongs to the Bisognano--Wichmann geometric modular class. Therefore the limiting modular automorphism group of \(\mathcal A(C)\) is implemented by the standard cap-preserving conformal one-parameter subgroup \(\lambda_C(s)\). Writing \(B_C\) for the infinitesimal generator supplied by that branch data, there exists \(\kappa_C>0\) such that
\[
K_C=\kappa_C B_C.
\]

Step 3: normalization. The modular flow is KMS at inverse temperature \(1\) by definition. Since the geometric parameter \(s\) in \(\lambda_C(s)\) is normalized by unit null dilation in the \textbf{T1} branch, the usual Bisognano--Wichmann relation identifies the modular parameter with the geometric one by \(s=2\pi t\) \cite{bw75,bw76,bgl93}. Equivalently,
\[
\kappa_C=2\pi.
\]
...
The theorem proves the controlled collar reduction together with the \(2\pi\) normalization \emph{on the OPH refinement branch in which the cap net has the Bisognano--Wichmann geometric modular action of \textbf{T1}}. On that branch, the standard cap-preserving conformal subgroup is part of the branch data, and the theorem identifies its modular generator with the carried collar remainder made explicit. The branch-selection question is encoded in the stated \textbf{T1} hypothesis; the theorem then fixes the normalization and controlled refinement limit on that branch.
\end{remark}
...
\Conf^+(S^2)\cong \mathrm{PSL}(2,\mathbb C)\cong \mathrm{SO}^+(3,1).
\]
By Theorem~\ref{thm:bw} together with the \textbf{T1} branch hypothesis, the cap modular groups are identified with the standard cap-preserving conformal dilations inside this group. Hence the local kinematics induced by modular flow is the connected Lorentz group.
\end{proof}
...
Phase II calibration sector (D10) & Pixel-closure gauge-coupling consistency, \(\alpha_i(m_Z)\), \(\alpha_{\mathrm{em}}^{-1}(m_Z)\), \(\sin^2\theta_W(m_Z)\), and the supplement-backed \(W/Z\) carrier diagnostics & Failure of the supplement-backed closure once \(P\) and the printed running/matching conventions are imposed \\
...
This is no more problematic than using a lattice regulator for Yang-Mills theory and then taking a continuum limit in which the operator algebra changes type. The mathematical burden is the existence of a controlled scaling regime, isolated in Assumption~\ref{ass:technical}.
...
The local MaxEnt branch gives a finite-range interacting fixed-cutoff dynamics, Propositions~\ref{prop:gauge} and \ref{prop:ancillauv} fix uniqueness of the UV branch only modulo OPH-stable equivalence, and Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} with Proposition~\ref{prop:hginteracting} close the genuinely noncentral topological branch at fixed cutoff. The open items on the UV side are the continuum BW/geometric lift of \textbf{T1}+\textbf{T3} and the fuller transportable compact-gauge lift when \textbf{T2} and \textbf{T5}--\textbf{T7} are invoked.

The D10 calibration sector gives supplement-backed gauge-coupling closure on the displayed carrier. That carrier closes its own exact \(W/Z\) chart, but exact PDG \(W/Z\) matching is not obtained on that carrier. The separate D6 cosmological-capacity relation remains an input-dependent corollary. The charged-lepton and Koide sectors remain phenomenological continuations rather than theorem-level spectra; the charged-lepton branch has an explicit same-carrier obstruction together with a two-scalar support-extension shell, not a closed emitted spectrum. On the flavor side, the structural output fixed before those continuations is the realized counting chain \(N_g=3\) then \(N_c=3\).

The light-quark and hadron sectors remain weaker. A recovered-core nonzero light-quark pure-\(B\) selector is not derived, so the light-quark split remains a D12 continuation branch. The hadron lane depends on nonperturbative runtime completion and stable-channel convergence rather than a closed theorem-level mass surface.
