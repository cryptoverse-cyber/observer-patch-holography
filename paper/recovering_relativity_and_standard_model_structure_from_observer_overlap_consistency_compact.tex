...
\]
on the same ordered eigenvalue family, but it remains same-family-only beneath the exact centered-readback / closed-common-shift frontier, the live same-label \(q_e\) readback, and the remaining determinant trace-lift theorem \(3\mu(r)=\sum_e M_e^{\mathrm{ch}}\log q_e(r)\) on the charged determinant channel. The quark lane has an exact same-family witness
\[
(u,d,s,c,b,t)=
...
The open source-facing theorem is still the landing from the D10 calibration descendant package \(B_{D10}(P)\) to theorem-grade physical charged data \(Y_e(P)\), or equivalently to the charged determinant-line section \(s_{\det}(P)\). More sharply, the live same-label readback already populates the uncentered excitation weights \(q_e=\sqrt{g_e d_e}\), so the source-side determinant character
\[
...
\]
or on the trace-lift scaffold
\[
N_{\det}(r)=3\mu(r)-\sum_e M_e^{\mathrm{ch}}\log q_e(r).
\]
