...
\textbf{D3} & Lorentz branch \(\Conf^+(S^2)\cong \mathrm{SO}^+(3,1)\) (Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry; conformal-group classification & T8 together with the tangent fixed-point / tangent-half-space comparison packaged in Theorem~\ref{thm:bw} & Phase I conditional scaling-limit theorem \\
...
The continuum reductions still use several historical shorthand labels, but not all of them are extra premises. The labels \textbf{T2}--\textbf{T3} now refer only to the branch-internal quasi-local propagation and bounded-interval endpoint control already derived from Axiom~\ref{ax:maxent}; likewise, whenever later sections speak of a ``refinement-stable'' sector colimit, that qualifier means persistence along the same Axiom~\ref{ax:maxent} branch rather than an additional categorical regularity axiom. The genuinely theorem-external items retained below are:
...
\item[\textbf{T4}] a controlled refinement/scaling family of cap states whose rescaled collar neighborhoods converge to an \(\mathrm{SO}(2)\)-invariant tangent fixed point of the local MaxEnt branch, so that cap modular flow admits the tangent-space comparison used in Theorem~\ref{thm:bw};
...
\begin{remark}
The five axioms fix the OPH core, and in the present packaging they already internalize the quasi-local propagation and refinement-stable notion of sector persistence used by the scaling-limit and gauge branches. The cap-modular geometric action and the \(2\pi\) KMS normalization are recovered in Theorem~\ref{thm:bw}; what remains genuinely theorem-external is the null half-sided inclusion step \textbf{T6}, the obstruction clause \textbf{T7} when transportability is invoked, the scaling-limit scope clause \textbf{T8}, fixed-cap stationarity \textbf{T9}, and the bosonic/symmetric/fiber-functor part of \textbf{T10}--\textbf{T12}. In particular, the Lorentz/null-modular/Einstein and compact-gauge statements below are still scaling-limit claims rather than fixed-cutoff matrix-algebra identities.
\end{remark}
...
\textbf{D3} & Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}: geometric modular flow and Lorentz branch & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry; \(\Conf^+(S^2)\cong \mathrm{SO}^+(3,1)\) & T8 together with the tangent fixed-point / tangent-half-space comparison packaged in Theorem~\ref{thm:bw} & conditional scaling-limit theorem \\
...
No theorem in the compact note is meant to hide standard mathematical imports or extra physical premises inside the phrase ``from the axioms.'' The authoritative unpacking of every such claim is the dependency DAG above. In particular, quasi-local propagation and the refinement-stable sector-persistence notion are now internal to the local finite-constraint MaxEnt branch, the cap-modular geometric action and its \(2\pi\) normalization are packaged inside Theorem~\ref{thm:bw}, and the remaining null half-sided inclusion, fixed-cap stationarity, and bosonic/fiber-functor hypotheses remain explicit. The only rows that use the two external continuous inputs are D6, D10, the separate order-of-magnitude neutrino estimate attached to D6, and downstream branches built from them.
...
\begin{theorem}[Cap modular flow from collar locality and tangent-half-space control]\label{thm:bw}
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}. Let \(C\subset S^2\) be a cap and let \(\omega_\delta^C\) be a controlled refinement/scaling family on collars of width \(\delta\) about \(\partial C\) with the following properties:
\begin{enumerate}[leftmargin=2em]
\item the collar exact-Markov modulus and the transported modular-additivity remainder of Proposition~\ref{prop:modulartransport} both vanish along the family, so the cap modular generator localizes to the collar in the scaling limit;
\item the local finite-constraint MaxEnt branch is \(\mathrm{SO}(2)\)-invariant around \(\partial C\), so the limiting collar generator commutes with the boundary stabilizer;
\item after rescaling by \(1/\delta\) near any smooth boundary point, the collar modular flow converges on local observables to the modular flow of the corresponding tangent half-space fixed point of that same MaxEnt branch, and that tangent fixed point obeys the standard Bisognano--Wichmann half-space theorem.
\end{enumerate}
Then in the resulting controlled refinement/scaling limit, the modular flow of \(C\) is the unique cap-preserving conformal dilation with KMS normalization \(2\pi\):
\[
K_C=2\pi B_C.
\]
\end{theorem}
...
\begin{proof}
By Proposition~\ref{prop:modulartransport}, exact collar modular-additivity identities are valid either at literal exact Markovity or along any controlled collar family whose exact-Markov modulus and transported logarithmic remainder tend to zero. Hypothesis (i) therefore gives a well-defined scaling-limit modular generator supported on an arbitrarily thin collar of \(\partial C\).

Hypothesis (ii) says that the limiting collar data are invariant under the \(\mathrm{SO}(2)\) stabilizer of the boundary circle. On \(S^2\), the connected conformal maps preserving \(C\) and commuting with that stabilizer form a one-parameter noncompact subgroup. Denote its infinitesimal generator by \(B_C\). Since the collar-supported modular flow preserves the cap algebra and commutes with the same boundary stabilizer, its scaling-limit generator must be proportional to \(B_C\):
\[
K_C=\beta_C B_C
\]
for some positive constant \(\beta_C\).

To determine \(\beta_C\), blow up any smooth point of \(\partial C\) by the collar scale \(\delta\). Hypothesis (iii) identifies the resulting tangent modular flow with the standard half-space modular flow of the tangent fixed point. The Bisognano--Wichmann theorem on that tangent half-space fixes the modular generator to be the boost generator with normalization \(2\pi\) \cite{BW75,BW76}. Because \(B_C\) reduces under the same blow-up to the tangent boost generator, the proportionality constant must be
\[
\beta_C=2\pi.
\]
The boundary circle is connected and the \(\mathrm{SO}(2)\)-invariant collar limit is the same at every boundary point, so the same normalization holds globally on \(C\). Hence \(K_C=2\pi B_C\).
\end{proof}
...
\begin{proof}
Theorem~\ref{thm:bw} identifies cap modular flow with the cap-preserving conformal dilation. Together with the manifest \(\mathrm{SO}(3)\) rotations of the screen, these generators lie in the orientation-preserving conformal group of \(S^2\). Standard conformal classification then gives
\[
\Conf^+(S^2)\cong \mathrm{PSL}(2,\mathbb C)\cong \mathrm{SO}^+(3,1),
\]
which is the connected Lorentz group.
\end{proof}
