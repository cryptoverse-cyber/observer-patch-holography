...
The continuum reductions additionally use the following shorthand package. Items \textbf{T2}--\textbf{T3} and the refinement-stable adjective inside \textbf{T12} are branch-internal consequences of Axiom~\ref{ax:maxent}; the remaining items are genuine extra premises:
...
\item the branch-internal finite-velocity quasi-local propagation bound just described;
\item the branch-internal endpoint-Lipschitz / finite-variation control for bounded null-interval modular Hamiltonians after removal of central endpoint terms;
...
\item whenever compact gauge reconstruction is invoked, the sector category means the directed colimit of transportable edge sectors with objectwise finite-dimensional fibers; the refinement-stable qualifier on that colimit is supplied internally by Axiom~\ref{ax:maxent}, not added as an independent categorical regularity condition.
\end{enumerate}
\end{assumption}
...
\begin{remark}
The five axioms fix the OPH core, and in the present packaging they already internalize the quasi-local propagation and refinement-stable notion of sector persistence used by the scaling-limit and gauge branches. What remains genuinely theorem-external is the geometric modular phase \textbf{T4}--\textbf{T5}, the null half-sided inclusion step \textbf{T6}, the obstruction clause \textbf{T7} when transportability is invoked, the scaling-limit scope clause \textbf{T8}, fixed-cap stationarity \textbf{T9}, and the bosonic/symmetric/fiber-functor part of \textbf{T10}--\textbf{T12}. In particular, the Lorentz/null-modular/Einstein and compact-gauge statements below are still scaling-limit claims rather than fixed-cutoff matrix-algebra identities.
\end{remark}
