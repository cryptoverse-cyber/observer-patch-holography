...
The historical regulator labels \(R0/R1\) are not a second axiom layer in the compact note. They are the fixed-cutoff realization of the finite patch-net and gauge-quotient data already introduced in Definition~\ref{def:patchnet}, Theorem~\ref{thm:holonomy}, and Proposition~\ref{prop:gauge}. This section makes that realization explicit, then states the collar decomposition and generalized-entropy consequences in that language.

\begin{proposition}[Regulated realization of local OPH data]\label{prop:regulatedrealization}
Fix a regulator-scale finite patch cover of a cap neighborhood and let \(R\) be any finite union of regulator cells. Then:
\begin{enumerate}[leftmargin=2em]
...
\]
\item the overlap-identification redundancy that leaves shared interface data invariant is supported on the cut degrees of freedom and, on each finite-dimensional chart, is represented by a compact unitary boundary gluing group \(G_{\partial R}\subset U(\tilde{\mathcal H}_R)\);
\item the physical region algebra is the fixed-point algebra
\[
\mathcal A(R)=\widetilde{\mathcal A}(R)^{G_{\partial R}}
=\mathcal B(\tilde{\mathcal H}_R)^{G_{\partial R}}.
\]
\end{enumerate}
Historical labels \(R0\) and \(R1\) are shorthand for items 1 and 3 in this realized regulator presentation.
\end{proposition}

\begin{proof}[Proof sketch]
Definition~\ref{def:patchnet} already introduces a fixed-cutoff patch net with finitely many local states. Choosing an orthonormal basis for each finite set \(S_i\) Hilbertizes it to \(\tilde{\mathcal H}_i\cong \mathbb C^{|S_i|}\), and finite unions of cells therefore carry finite-dimensional extended algebras. Proposition~\ref{prop:gauge} identifies gauge as the redundancy that preserves interface data, while Theorem~\ref{thm:holonomy} shows that the nontrivial obstruction data live on loops and hence on boundary gluing. On a finite-dimensional extended Hilbert space, any unitary image of that boundary redundancy has compact closure. Physical observables are precisely the endomorphisms that descend through the quotient by that redundancy, so they form the fixed-point algebra of the resulting compact boundary action.
\end{proof}

\begin{theorem}[Edge-center collar decomposition and exact Markov normal form]\label{thm:markovcollar}
Let \(A\)-\(B\)-\(D\) be a collar tripartition realized at fixed regulator scale as in Proposition~\ref{prop:regulatedrealization}. Write \(B=B_L\cup B_R\) with common interface \(\Sigma=\partial C\), let
\[
\mathcal H_B=(\tilde{\mathcal H}_{B_L}\otimes \tilde{\mathcal H}_{B_R})^{G_\Sigma},
\]
and decompose the left and right extended boundary data as
\[
\tilde{\mathcal H}_{B_L}\cong \bigoplus_\alpha V_\alpha\otimes \mathcal H_{b_L^\alpha},
\qquad
\tilde{\mathcal H}_{B_R}\cong \bigoplus_\beta V_\beta^*\otimes \mathcal H_{b_R^\beta},
\]
with \(V_\alpha\) irreducible \(G_\Sigma\)-modules. Then
\[
...
\]
The label \(\alpha\) is the classical edge-center data carried by the cut.
\end{theorem}

\begin{proof}
By Proposition~\ref{prop:regulatedrealization}, the collar algebra is the fixed-point algebra of the diagonal boundary gluing action on \(\tilde{\mathcal H}_{B_L}\otimes \tilde{\mathcal H}_{B_R}\). Therefore
\[
...
\end{cases}
\]
so the invariant collar Hilbert space has the displayed block form and its center is generated by the block projectors. Within each block, observables from \(A\cup B\) act only on the left factor and observables from \(B\cup D\) act only on the right factor. If the state is exact Markov, the standard HJPW structure theorem gives the blockwise factorized normal form \cite{HJPW}. The same formula is used in the idealized recoverability limit when exact identities are taken literally.
\end{proof}

\begin{remark}[Status of the regulator package]
In the compact note, Assumption~\ref{ass:technical}\textbf{T1} is read only as theorem-local shorthand for the fixed-cutoff realization above. The nontrivial open burden is the controlled refinement/scaling regime of Assumption~\ref{ass:technical}\textbf{T8} and the later modular/null-limit premises, not the matrix-algebra bookkeeping at one regulator scale.
\end{remark}
