...
The c.12-local bridge now ends with an explicit theorem-level object: the positive self-adjoint null-translation generator \(P_\Omega\) on its Stone domain, together with its Borchers affine covariance relation and the half-line modular identities above. What remains unproved here is exactly what the compact note marks as downstream: the density upgrade from the weak endpoint derivative to a local operator-valued distribution, the relativistic identification of that density with continuum \(T_{kk}\), and any bounded-interval modular-Hamiltonian formula beyond the separate interval-preserving projective branch isolated in Theorem~\ref{thm:stress}. In particular, positivity and unitary implementation of null translations are no longer packaged as an inferred endpoint or an imported physical premise, whereas the density upgrade and null-stress identification still are.
\end{remark}
