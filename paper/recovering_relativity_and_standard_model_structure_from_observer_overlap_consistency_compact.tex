...
\begin{enumerate}[leftmargin=2em]
\item A conditional Jacobson-type Einstein branch is recovered from observer-overlap modular geometry, an explicit fixed-cap generalized-entropy stationarity premise, and an explicit null-modular / null-stress scaling-limit package; in the synchronized c.11 bridge, quasi-local propagation, endpoint control, half-sided inclusion, and Borchers positivity are now supplied internally through the positive null-translation stage, while the density upgrade and null-stress identification remain explicit downstream burdens.
...
\item[\textbf{T4}] the branch condition that the refinement-stable cap net lies in the geometric modular class for caps and their conformal images, together with the SO(2)-equivariance / cap-isotropy selector that picks the standard cap-preserving conformal dilation inside the circle stabilizer, so that Theorem~\ref{thm:bw} can identify the limiting modular flow with that dilation;
...
\begin{remark}
The five axioms fix the OPH core, and in the present packaging they already internalize the quasi-local propagation and refinement-stable notion of sector persistence used by the scaling-limit and gauge branches. On the gravity side this means the null bridge no longer imports a separate propagation, endpoint-control, or half-sided-inclusion premise: c.11 closes the fixed-cutoff strip argument through exact-or-controlled strip additivity, the weak tail generator, and the derived half-sided modular pair that Borchers--Wiesbrock promotes to a positive null translation generator. The cap-modular geometric action and the \(2\pi\) KMS normalization are recovered in Theorem~\ref{thm:bw} once the OPH geometric branch and its SO(2)-equivariance selector are imposed; what remains genuinely theorem-external is the retained eight-item technical package: the obstruction clause \textbf{T7} when transportability is invoked, the scaling-limit scope clause \textbf{T8}, fixed-cap stationarity \textbf{T9}, and the bosonic/symmetric/fiber-functor part of \textbf{T10}--\textbf{T12}, together with \textbf{T1} and \textbf{T4} where the fixed-cutoff regulator presentation and the geometric cap-modular branch hypothesis are used. In particular, the Lorentz/null-modular/Einstein and compact-gauge statements below are still scaling-limit claims rather than fixed-cutoff matrix-algebra identities.
\end{remark}
...
The collar analysis identifies where modular nonadditivity can live, but by itself it does not determine the continuum geometric action of the limiting modular group. The correct theorem-scope is therefore branch-conditional and must be stated explicitly. What OPH supplies here is a controlled reduction from the regulator collar to the refinement limit, together with a uniqueness and normalization statement \emph{once} the refinement-stable cap net lies in the geometric modular class. This removes the earlier proof-sketch packaging in which cap isotropy and Euclidean regularity appeared as separate imported premises while leaving the actual remaining phase-selection burden visible.
...
\begin{theorem}[Geometric modular flow on caps in the OPH geometric branch]\label{thm:bw}
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, together with the fixed-cutoff regulator presentation \textbf{T1}, the branch-internal control statements denoted \textbf{T2}--\textbf{T3}, and the controlled scaling-limit scope clause \textbf{T8} of Assumption~\ref{ass:technical}. Assume in addition that the refinement-stable MaxEnt branch selected by OPH lies in the geometric modular class for caps and their conformal images, and that in this branch the cap modular flow commutes with the \(\mathrm{SO}(2)\) rotation subgroup fixing the round entangling circle. Then for every cap \(C\subset S^2\) the following hold.
...
\item In the refinement limit, the modular action on \(\mathcal A(C)\) is a one-parameter subgroup of \(\Conf^+(S^2)\) preserving \(C\), preserving \(\partial C\), and commuting with the cap's \(\mathrm{SO}(2)\) rotation subgroup.
\item The connected noncompact subgroup of \(\Conf^+(S^2)\) with that property is unique; if \(B_C\) denotes its generator, then the limiting modular Hamiltonian is of the form
\[
K_C=\kappa_C\,B_C
\]
for some \(\kappa_C>0\).
...
Step 2: passage to the geometric branch. By the branch hypothesis, the refinement-stable cap net belongs to the geometric modular class. Hence the limiting modular automorphism group of \(\mathcal A(C)\) acts by orientation-preserving conformal transformations of \(S^2\) and preserves the cap \(C\). Since the modular group also preserves the algebra of the complement, it preserves the boundary circle \(\partial C\). By the additional selector packaged in \textbf{T4}, this limiting cap modular flow also commutes with the \(\mathrm{SO}(2)\) rotation subgroup fixing that round circle.

Step 3: uniqueness of the cap-preserving noncompact flow. The stabilizer in \(\Conf^+(S^2)\) of a round circle is conjugate to the stabilizer of the equator in the M\"obius group. Inside that circle stabilizer, the requirement that the flow preserve one chosen cap and commute with the \(\mathrm{SO}(2)\) rotations about the cap axis singles out the standard hyperbolic dilation uniquely. Denote its infinitesimal generator by \(B_C\). Therefore the limiting modular Hamiltonian must be proportional to \(B_C\):
\[
K_C=\kappa_C B_C
\]
for some positive constant \(\kappa_C\).

Step 4: normalization. The modular flow is KMS at inverse temperature \(1\) by definition. After analytic continuation to Euclidean modular angle, the orbit of the geometric cap-preserving flow closes with the standard angular period. Therefore the proportionality constant is fixed uniquely to
\[
\kappa_C=2\pi.
\]
This is the same rigidity that fixes the standard Bisognano--Wichmann normalization; here it is stated as a property of the geometric OPH branch rather than as a separate imported Euclidean premise.

Step 5: controlled limit. At finite collar scale the only discrepancy between the physical modular generator and its exact geometric normal form is the carried collar-replacement error already isolated above. Hence one may write
\[
K_C^{(\delta)}=2\pi B_C+E_C^{(\delta)}
\]
...
The theorem proves geometric modular flow and its \(2\pi\) normalization \emph{on the OPH refinement branch in which the cap net has geometric modular action}. The attached source set does not prove that the bare OPH axioms alone force the existence of that branch. The remaining nontrivial burden is therefore phase selection in the refinement limit, not the collar-local reduction or the normalization once the geometric branch is realized.
\end{remark}
...
\Conf^+(S^2)\cong \mathrm{PSL}(2,\mathbb C)\cong \mathrm{SO}^+(3,1).
\]
By Theorem~\ref{thm:bw}, the cap modular groups are identified with the cap-preserving conformal dilations inside this group. Hence the local kinematics induced by modular flow is the connected Lorentz group.
\end{proof}

\subsection{The null modular bridge}

The fixed-cutoff strip analysis still begins from the transferred cut-center data and the inherited-strip Markov structure, but the bridge now advances one step farther than the c.10 boundary. At fixed regulator scale one obtains exact or controlled four-term strip additivity together with endpoint-Lipschitz control of the renormalized half-line family; on the same OPH geometric scaling branch used in Theorem~\ref{thm:bw}, the null half-line blow-up net then inherits geometric dilation and therefore half-sided modular inclusion. Standard Borchers--Wiesbrock theory may consequently be applied as imported mathematics to a now-derived OPH half-sided modular pair, so positivity and unitary implementation of null translations are no longer extra physical premises. What is still not proved here is the downstream density upgrade from the weak tail generator to a local operator-valued density and the relativistic identification of that density with continuum \(T_{kk}\).
\par
...
\end{enumerate}
The labels \(\ref{lem:density}\) and \(\ref{thm:stress}\) are retained below as the only explicit downstream boundaries. Historical references elsewhere to the old null half-sided-inclusion label \textbf{T6} should, within this subsection, be read as shorthand for the derived statement of Corollary~\ref{cor:n2}.
...
\]
The object \(q(a,\Omega)\) is the weak tail generator. At this stage it is not yet asserted to be positive or self-adjointly implemented.
\end{proposition}
...
\begin{lemma}[Borchers promotion to a genuine null translation generator]\label{lem:translation}
For the derived half-sided modular pair of Corollary~\ref{cor:n2}, the Borchers--Wiesbrock theorem yields a strongly continuous one-parameter unitary group \(U_\Omega(a)=e^{iaP_\Omega}\), \(a\in\mathbb R\), with \(P_\Omega\ge 0\), such that for \(a,b\ge 0\),
\[
U_\Omega(a)\,\mathcal M_b(\Omega)\,U_\Omega(a)^*=\mathcal M_{a+b}(\Omega),
\]
and
\[
\Delta^{it}U_\Omega(a)\Delta^{-it}=U_\Omega(e^{-2\pi t}a),
\qquad
[K,P_\Omega]=i2\pi P_\Omega.
\]
The weak tail generator of Proposition~\ref{prop:n3} is therefore upgraded from a domain-level endpoint derivative to a genuine positive translation generator.
\end{lemma}

\begin{proof}
Apply the standard Borchers--Wiesbrock theorem to the half-sided modular inclusion
\[
\mathcal M_a(\Omega)\subset \mathcal M_0(\Omega)
\]
supplied by Corollary~\ref{cor:n2}. That theorem produces a unique positive self-adjoint generator \(P_\Omega\) and a strongly continuous unitary representation \(U_\Omega(a)=e^{iaP_\Omega}\) whose action translates the nested half-line net and obeys the affine covariance relation with the modular group. Differentiating
\[
\Delta^{it}U_\Omega(a)\Delta^{-it}=U_\Omega(e^{-2\pi t}a)
\]
at \(a=0\) gives
...
This theorem is not yet established at the present c.11 step; what remains beyond the derived half-sided-inclusion bridge is precisely the density upgrade of Theorem~\ref{lem:density} and the relativistic identification of that density with continuum null stress.
\end{theorem}
...
The c.11-local bridge is now closed through the derived half-sided modular-inclusion step. What has been proved here is the fixed-cutoff transfer of cut-center data to null strips, the exact-or-controlled four-term strip relation on one inherited strip model, endpoint-Lipschitz control of the renormalized half-line family strong enough to define the weak tail generator \(q(a,\Omega)\), and the geometric blow-up argument that yields half-sided modular inclusion and hence a positive null-translation generator by Borchers--Wiesbrock. The density upgrade of \(q\) to a local operator-valued density and the continuum null-stress identification remain explicit downstream tasks rather than hidden proof sketches.
\end{remark}
...
\]
The logic is simple: once $\lambda(M_U)=0$, the condition $\beta_\lambda(M_U)=0$ removes the final freedom in the top Yukawa boundary value. The low-energy Higgs and top masses then follow inside the synchronized supplement-backed D11 implementation rather than by direct fitting. In the compact note, the displayed one-loop equations record the core RG transport conventions only; the quoted D11 numbers also use the matching, extraction, and pole-mass conventions fixed in the supplement-backed implementation.

Within that synchronized supplement-backed D11 implementation, running the coupled system $(y_t,\lambda,g_Y,g_2,g_3)$ down to $\mu\sim m_t$ yields
\[
...
\]
The appendix is therefore not intended as a standalone reproduction recipe for the displayed $126.5$ and $171.1$ GeV outputs; it records the one-loop flow equations used inside the broader synchronized D11 implementation.
...
For convenience we record the core one-loop system used in the Higgs/top transport branch. This appendix is not by itself a full standalone reproduction recipe for the displayed D11 numbers: the synchronized supplement-backed implementation also fixes the matching scale, boundary synchronization, low-scale extraction conventions, and the pole-mass conversion choices used for the quoted outputs.
...
\end{align}
In the present supplement-backed numerical implementation this system supplies the core transport layer between the UV boundary data and the low-energy Higgs/top outputs. Numerically, the compact note uses one-loop $\overline{\mathrm{MS}}$ transport, the matching scale $\mu=m_Z=91.1876\,\mathrm{GeV}$, the unified scale $M_U\approx 2.474\times 10^{16}\,\mathrm{GeV}$ from the main text, the synchronized low-scale extraction convention for $\lambda(m_t)$, and the standard three-loop QCD pole-mass conversion for the displayed top-branch number. Higher-loop and threshold refinements are expected to shift the final numbers at the GeV level rather than alter the logic of the branch.
