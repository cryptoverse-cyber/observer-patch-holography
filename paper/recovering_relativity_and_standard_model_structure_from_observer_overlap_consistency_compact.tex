...
\item[\textbf{T4}] the branch condition that the refinement-stable cap net lies in the geometric modular class for caps and their conformal images, so that Theorem~\ref{thm:bw} can identify the limiting modular flow with the unique cap-preserving conformal dilation;
\item[\textbf{T6}] a standard half-sided modular inclusion premise for the null half-line blow-up algebras;
...
\begin{remark}
The five axioms fix the OPH core, and in the present packaging they already internalize the quasi-local propagation and refinement-stable notion of sector persistence used by the scaling-limit and gauge branches. The cap-modular geometric action and the \(2\pi\) KMS normalization are recovered in Theorem~\ref{thm:bw} on the OPH geometric branch; what remains genuinely theorem-external is the retained nine-item technical package: the null half-sided inclusion step \textbf{T6}, the obstruction clause \textbf{T7} when transportability is invoked, the scaling-limit scope clause \textbf{T8}, fixed-cap stationarity \textbf{T9}, and the bosonic/symmetric/fiber-functor part of \textbf{T10}--\textbf{T12}, together with \textbf{T1} and \textbf{T4} where the fixed-cutoff regulator presentation and the geometric cap-modular branch hypothesis are used. In particular, the Lorentz/null-modular/Einstein and compact-gauge statements below are still scaling-limit claims rather than fixed-cutoff matrix-algebra identities.
\end{remark}
...
\textbf{D3} & Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}: geometric modular flow and Lorentz branch & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry; \(\Conf^+(S^2)\cong \mathrm{SO}^+(3,1)\) & T8 together with the OPH geometric-branch hypothesis packaged in Theorem~\ref{thm:bw} & conditional scaling-limit theorem \\
\textbf{D4} & Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Theorem~\ref{lem:density}, Lemma~\ref{lem:translation}; Theorem~\ref{thm:stress}: null modular bridge to \(T_{kk}\) & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock half-sided modular inclusion; distributional differentiation on half-lines & T1, T6, and T8 together with the relativistic null-stress identification premise; the propagation controls denoted T2--T3 are supplied internally by Axiom~\ref{ax:maxent} on the local finite-constraint branch & conditional bridge theorem \\
...
\item In the refinement limit, the modular action on \(\mathcal A(C)\) is a one-parameter subgroup of \(\Conf^+(S^2)\) preserving \(C\) and fixing \(\partial C\).
...
Step 2: passage to the geometric branch. By the branch hypothesis, the refinement-stable cap net belongs to the geometric modular class. Hence the limiting modular automorphism group of \(\mathcal A(C)\) acts by orientation-preserving conformal transformations of \(S^2\) and preserves the cap \(C\). Since the modular group also preserves the algebra of the complement, it fixes the boundary circle \(\partial C\).

Step 3: uniqueness of the cap-preserving noncompact flow. The stabilizer in \(\Conf^+(S^2)\) of a round circle is conjugate to the stabilizer of the equator in the M\"obius group. Its connected noncompact one-parameter subgroup preserving one of the two caps is unique. Denote its infinitesimal generator by \(B_C\). Therefore the limiting modular Hamiltonian must be proportional to \(B_C\):
\[
K_C=\kappa_C B_C
\]
...
Under the derived quasi-local propagation and bounded-interval endpoint control of Axiom~\ref{ax:maxent}, matrix elements of $K[I]$ obey the interval-length domination estimate
\[
...
\begin{proof}[Proof sketch]
Local MaxEnt gives a quasi-local Gibbs form for the regulated state. Quasi-locality together with the finite-velocity propagation bound derived in Section~\ref{sec:premise-ledger} implies that moving an interval endpoint by $\Delta v$ changes expectation values of local observables by $O(\Delta v)$ after smearing, while the derived bounded-interval endpoint control from the same section supplies the required Lipschitz estimate. The resulting matrix elements are therefore Lipschitz on bounded intervals and in particular have finite variation.
\end{proof}
