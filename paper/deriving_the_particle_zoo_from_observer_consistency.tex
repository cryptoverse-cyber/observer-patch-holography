...
\begin{abstract}
Observer-Patch Holography (OPH) does not begin with a list of particles and then ask how to fit
them. It asks whether the observed particle world can be reconstructed from overlap consistency,
gauge reconstruction, and one shared calibration scale. This paper is the current status report on
that particle-spectrum program.

Starting from the OPH structural theorem package and the dimensionless pixel constant
\[
P \equiv a_{\mathrm{cell}}/\ell_P^2,
\]
the paper asks how far one can push the observed spectrum using one shared calibration constant
rather than the long fitted parameter list of the Standard Model. The answer is mixed but already
nontrivial. The photon, gluons, and graviton appear as structural massless carriers; the
electroweak stage yields the \(W\) boson and \(Z\) boson; the Higgs/top stage yields the Higgs
boson and top quark; and the quark branch emits continuation-level rows for the up, down,
strange, charm, and bottom quarks. Charged leptons, flavor-labeled neutrinos, and hadrons remain
open.

The purpose of the paper is to show the full derivation chain in one place, from the OPH axioms
and the shared pixel scale \(P\) to the present particle outputs, while keeping the remaining
closure objects visible rather than hiding them.
\end{abstract}
...
the counting results \(N_g=3\), \(N_c=3\). The present paper uses those results as input and
studies the downstream particle-spectrum continuation.
...
\subsection{Origin of the realized particle content}

In OPH, particles are not introduced as an independent list of elementary ingredients. They are
the observer-visible excitations and carrier modes that remain once three layers of structure have
been fixed:
...
\emph{Recovering Relativity and Standard Model Structure from Observer Overlap
Consistency}~\cite{ophcompact} provides the structural input used here. It fixes the realized
low-energy package
\[
\frac{\SU(3)\times\SU(2)\times\U(1)}{\mathbb Z_6},
\qquad
...
N_c=3,
\]
together with one admissible Higgs doublet and the realized chiral matter content. The particle
paper then asks how the realized gauge-and-matter branch constrains the low-energy carrier and
matter content.
...
hiding. Some of the nontrivial overlap structure is therefore not ``noise''; it is the
...
stable-channel readout pipeline. The hadron derivation is therefore the most simulation-heavy and
least publicly promoted part of the particle-spectrum story.
...
This section records the conceptual structure used throughout the paper: particle families are
treated as stabilized descendants of transport data on an observer network over \(S^2\), and the
later sections state how far that structure has been converted into explicit derivations and
reported outputs.

\section{Calibration and the Reverse-Engineering Strategy}

The reverse-engineering claim can be stated directly. In ordinary phenomenological use, the
Standard Model does not derive the observed particle masses and mixings from one common
microscopic constant. It is usually presented with \(19\) free parameters
...
The present OPH formulation still carries the
...
The structural OPH theorems determine \emph{what kind} of particle world is
realized, without yet fixing \emph{the absolute scale in GeV} that world occupies. The overlap,
...
Calibration is therefore central to the particle program. If different calibration observables
demand different implied pixel constants, then the electroweak stage is not yet an exact
single-\(P\) closure surface. In that case the task is
...
away that exact single-\(P\) closure fails. The bottleneck is a transport/matching problem on the
electroweak stage, rather than a lack of extra digits
...
The particle derivation contains three kinds of rows: structurally fixed rows, numerically emitted
rows, and rows whose derivation stops short of a promotable mass prediction. This section
summarizes that split.
...
Electroweak bosons & calibration & electroweak calibration stage & \(W\) boson, \(Z\) boson & repair law beyond the present electroweak carrier \\
...
Quark family & continuation & forward Yukawa derivation & up, down, strange, charm, bottom quarks & light-quark overlap selector, Yukawa dictionary, CKM/CP closure \\
Charged leptons & continuation & ordered-log and support-extension derivation & no promoted masses & \(\eta\) source readback, \(\sigma\) support-extension scalar, excitation dictionary \\
Neutrinos & continuation & intrinsic Majorana eta-chain & no promoted masses & same-label pullback closure and PMNS closure \\
Hadrons & simulation / nonperturbative continuation & stable-channel correlator and readout derivation & no promoted hadron masses & runtime receipt, executed unquenching, and production systematics \\
\bottomrule
\end{tabular}
\end{table}

Only the quark family currently has emitted matter-sector numerical rows, and those rows remain
continuation-level rather than theorem-closed. The bosonic rows also split by tier: the photon,
gluons, and graviton are structural zeros; the \(W\) boson and \(Z\) boson arise at the
electroweak calibration stage; and the Higgs boson belongs to the Higgs/top critical stage. This
table sets the status boundary for the rest of the paper.
...
Electroweak calibration & \(W\) boson & calibration & \(80.3862917~\mathrm{GeV}\) & derive the repair value law beyond the present electroweak carrier \\
Electroweak calibration & \(Z\) boson & calibration & \(91.1829044~\mathrm{GeV}\) & same electroweak repair/value-law frontier as the \(W\) row \\
...
Quark family & up quark & overlap continuation & reported value \(0.00228766~\mathrm{GeV}\); stronger overlap-branch candidate \(0.00217663~\mathrm{GeV}\) & derive the selector/value law and the CKM generator \(K_{\mathrm{CKM}}\) \\
Quark family & down quark & overlap continuation & reported value \(0.00512144~\mathrm{GeV}\); stronger overlap-branch candidate \(0.00470823~\mathrm{GeV}\) & same selector/value-law plus \(K_{\mathrm{CKM}}\) frontier \\
Quark family & strange quark & overlap continuation & reported value \(0.0916111~\mathrm{GeV}\); stronger overlap-branch candidate \(0.0916083~\mathrm{GeV}\) & same selector/value-law plus \(K_{\mathrm{CKM}}\) frontier \\
Quark family & charm quark & overlap continuation & reported value \(1.256692~\mathrm{GeV}\); stronger overlap-branch candidate \(1.256692~\mathrm{GeV}\) & same selector/value-law plus \(K_{\mathrm{CKM}}\) frontier \\
Quark family & bottom quark & overlap continuation & reported value \(3.820118~\mathrm{GeV}\); stronger overlap-branch candidate \(4.155514~\mathrm{GeV}\) & same selector/value-law plus \(K_{\mathrm{CKM}}\) frontier \\
Charged leptons & electron & continuation gap & \(n/a\); continuation pair \((\eta,\sigma)=(-6.72959,\,8.15406)\) with near-exact centered-log shape closure & emit theorem-grade \(\eta\), then \(\sigma\), then the absolute scale \(g_e\) \\
Charged leptons & muon & continuation gap & \(n/a\); same continuation bridge as the electron row & same \(\eta \rightarrow \sigma \rightarrow g_e\) closure chain \\
Charged leptons & tau lepton & continuation gap & \(n/a\); same continuation bridge as the electron row & same \(\eta \rightarrow \sigma \rightarrow g_e\) closure chain \\
Neutrinos & \(\nu_e\) & continuation gap & \(n/a\); intrinsic eta-demo eigenmasses \((2.393,\,2.4048,\,2.5896)\times 10^{-12}~\mathrm{GeV}\) are exact once the centered eta-class is fixed & emit the proof-facing eta-class and the shared charged-lepton left basis \\
...
Hadrons & proton & runtime / systematics gap & no promoted proton row; surrogate finest-ensemble \(N_{\mathrm{iso}}\) candidate \(0.938960210578~\mathrm{GeV}\) & replace the surrogate bridge with production unquenched execution and production systematics \\
Hadrons & neutron & runtime / systematics gap & no promoted neutron row & production unquenched execution plus isospin-resolved hadron systematics \\
Hadrons & neutral pion proxy & runtime / systematics gap & no promoted pion row; surrogate finest-ensemble \(\pi_{\mathrm{iso}}\) candidate \(0.135039383836~\mathrm{GeV}\) & replace the surrogate bridge with production unquenched execution and production systematics \\
Hadrons & \(\rho(770)^0\) proxy & runtime / systematics gap & no promoted resonance row & production execution plus finite-volume resonance extraction \\
\end{longtable}
...
The compact paper states the axioms in algebraic language. The screen-microphysics work
...
First, it makes the screen picture operational. A patch is a finite local algebra carried by one
region of the cellulated screen, and an overlap is a boundary-visible algebra with declared shared
...
theorem-bearing fixed-cutoff statement about a central record
algebra, Born probabilities for its event projectors, and L\"uders conditioning on that same
commuting algebra.

The regulated architecture serves as a reference implementation. In the present suite the
physically meaningful uniqueness statement is quotient-level and ancilla-stable: gauge-covariant
repair fixes the physical branch modulo implementation hiding and inert ancillary refinement, not
a unique microscopic screen presentation. The microphysics paper selects one finite
...
language, while leaving microscopic uniqueness and the continuum lift open.
...
The particle-spectrum paper needs to keep these inputs visible because several later sections
...
The third axiom internalizes several pieces of infrastructure that earlier presentations often
...
Hilbert spaces and a boundary-fixed overlap action on cut data. On top of that, the still-genuine extra burdens are the controlled scaling-limit
...
For UV language this means interaction at fixed cutoff is already internal to the local MaxEnt
branch, observable uniqueness is only modulo gauge and inert ancillary refinement, and the BW
lift plus the transportable compact-gauge lift remain behind the explicit T1/T3 and
T2/T5--T7 boundaries. The genuinely noncentral topological case is now closed at fixed cutoff
by a higher-gauge crossed-module edge-center and transport package rather than left as an
ordinary-group-proof gap.
...
theorem packages imported into the present paper are the following.
...
supplement and the observer paper, supplies the fixed-cutoff central-record, Born-rule, and
...
OPH gravity chain. They are symmetry-protected zeros of the realized branch rather than
continuation-level masses.
...
then a broader repair question beyond that carrier if the selected point does not
coincide with the PDG pair. In practical terms, the construction starts from
the declared pixel input \(P\), builds the running electroweak family, reduces it to the displayed
...
\[
m_W = 80.3862917~\mathrm{GeV},
\qquad
m_Z = 91.1829044~\mathrm{GeV}.
\]
Those numbers are not presented as arbitrary-decimal exact closure. The exact carrier
chart is closed on the selected pair, but exact PDG \(W/Z\) does not honestly follow on the displayed carrier from the emitted selector
package. The correct reading is therefore that the electroweak carrier certifies
its own obstruction, and the remaining problem is a repair problem beyond that
carrier branch. That is a sharper and more honest diagnosis than merely saying that more
decimal precision in \(P\) is required.

\begin{theorem}[Frozen-target point uniqueness on the electroweak repair chart]
Fix the emitted electroweak repair-chart carrier basis
\[
(\alpha_{Y,m_Z},\alpha_{2,m_Z},\eta_{\mathrm{source}},v_{\mathrm{inherited}}),
\]
and define
\[
\beta_{\mathrm{EW}}
=
\frac{\alpha_{2,m_Z}-\alpha_{Y,m_Z}}
     {\alpha_{2,m_Z}+\alpha_{Y,m_Z}},
\qquad
n_{\mathrm{EW}}^{\mathrm{fiber}}(\tau_2)
=
1+
\frac{\beta_{\mathrm{EW}}\tau_2
+2(1+\beta_{\mathrm{EW}})\tau_2^3
-(1-\beta_{\mathrm{EW}})\eta_{\mathrm{source}}}
     {1+4\tau_2^2}.
\]
For any admissible frozen target pair
\((M_W^{\mathrm{target}},M_Z^{\mathrm{target}})\),
the repair chart has exactly one point
\((\tau_{2,\mathrm{target}},\delta n_{\mathrm{target}})\)
that emits that pair, namely
\[
\tau_{2,\mathrm{target}}
=
\frac{M_W^{\mathrm{target}\,2}}
     {\pi v_{\mathrm{inherited}}^2 \alpha_{2,m_Z}} - 1,
\]
\[
\delta n_{\mathrm{target}}
=
\frac{M_Z^{\mathrm{target}\,2}}
     {\pi v_{\mathrm{inherited}}^2(\alpha_{Y,m_Z}+\alpha_{2,m_Z})}
-
n_{\mathrm{EW}}^{\mathrm{fiber}}(\tau_{2,\mathrm{target}}).
\]
Equivalently,
\[
\delta\alpha_{2}^{\mathrm{target}}
=
\alpha_{2,m_Z}\tau_{2,\mathrm{target}},
\]
\[
\delta\alpha_{Y}^{\mathrm{target}}
=
\alpha_{Y,m_Z}
\frac{8\eta_{\mathrm{source}}\tau_{2,\mathrm{target}}^2-\tau_{2,\mathrm{target}}}
     {1+4\tau_{2,\mathrm{target}}^2}
+
(\alpha_{Y,m_Z}+\alpha_{2,m_Z})\delta n_{\mathrm{target}}.
\]
Hence, once the target specification is frozen, the remaining electroweak burden is not another chart
search but the OPH law that emits this unique point without inverse fitting.
\end{theorem}

Two diagnostic target freezes are informative. For the pair displayed in this paper, the unique target point is
\[
\tau_{2,\mathrm{target}}=-2.311626831634772\times10^{-4},
\qquad
\delta n_{\mathrm{target}}=2.346363903116266\times10^{-4}.
\]
For the reference PDG pair
\[
m_W = 80.3692\pm0.0133~\mathrm{GeV},
\qquad
m_Z = 91.1880\pm0.0020~\mathrm{GeV},
\]
the corresponding target point is
\[
\tau_{2,\mathrm{target}}=-4.251937754427004\times10^{-4},
\qquad
\delta n_{\mathrm{target}}=3.395982887997029\times10^{-4}.
\]
So exactness is not merely numerically open; it is also ill-posed until the electroweak calibration
target pair itself is frozen at one declared precision.
...
\midrule
\(W\) boson & electroweak calibration & calibration & \(80.3862917~\mathrm{GeV}\) \\
\(Z\) boson & electroweak calibration & calibration & \(91.1829044~\mathrm{GeV}\) \\
...
\subsection{Strongest overlap-branch mass-side primitive}

The strongest constructive continuation object is a one-scalar light-sector overlap law.
Write
\[
...
\sigma_d = 3.3049.
\]
The selector-value candidate used on this continuation branch is
\[
\Delta_{ud}^{\mathrm{overlap}}
=
\frac{t_1}{5},
\qquad
t_1 = 0.6695617711471163,
\]
so
\[
\Delta_{ud}^{\mathrm{overlap}} = 0.13391235422942326.
\]
That candidate yields the noncircular mass branch
\[
(u,c,t)
=
(0.002176632493,\ 1.256692171439,\ 172.851929939314)\ \mathrm{GeV},
\]
\[
(d,s,b)
=
(0.004708229529,\ 0.091608271273,\ 4.155513989985)\ \mathrm{GeV},
\]
with RMS log-mass error
\[
\sqrt{\frac16\sum_q \log^2\!\frac{m_q^{\mathrm{cand}}}{m_q^{\mathrm{ref}}}}
\approx
0.010784.
\]
The best one-scalar point on the same family is nearby,
\[
\Delta_{ud}^{\mathrm{overlap}} \approx 0.1404,
\qquad
\Lambda_{ud}^{B,\mathrm{transport}} \approx 0.145808,
\]
with RMS log error \(\approx 0.010421\).

So the mass branch is materially advanced, but CKM/CP still does not close on this one-scalar
family. At the \(t_1/5\) mass point one finds only
\[
|V_{us}| \approx 0.00760,
\qquad
|V_{cb}| \approx 0.00129,
\qquad
J \approx 2.88\times10^{-10},
\]
far below the observed CKM/CP scales. The strongest smaller constructive residual is therefore
matrix-valued rather than scalar:
\[
K_{\mathrm{CKM}}
=
\log\!\left(
U_u^{\mathrm{left}}(\Delta)\,
V_{\mathrm{CKM}}^{\mathrm{target}}\,
U_d^{\mathrm{left}}(\Delta)^\dagger
\right).
\]
At the candidate mass point its physical off-diagonal magnitudes are
\[
|K_{12}| = 0.225515694,
\qquad
|K_{23}| = 0.043789773,
...
|K_{13}| = 0.003472634.
\]
The remaining quark frontier has two components:
\begin{enumerate}[leftmargin=2em]
...
\item the emitted pure-\(B\) payload pair beneath the reported quark values is still open;
\item a broader open frontier remains in the light-quark isospin-breaking selector /
overlap-defect scalar together with textures, CKM mixing, and CP.
\end{enumerate}

The correct headline for quarks is that they are the only matter family with reported numerical
rows, but the row-emission branch still sits above visible open
...
of the charged-lepton chapter is to explain how those three rows emerge from the shared flavor dictionary without falling back to Koide-assisted fitting. The two
remaining mathematical tasks are deriving the Yukawa/excitation
dictionary and replacing Koide-assisted lepton fitting with a theorem.
...
The scalar order remains
\[
\eta_{\mathrm{ext}}
\quad\text{then}\quad
\sigma_{\mathrm{ext}},
\]
and the charged absolute scale \(g_e\) remains unresolved.
...
the continuation branch, but the theorem-grade derivation still lacks emitted
\(\eta_{\mathrm{ext}}\), emitted \(\sigma_{\mathrm{ext}}\), and an OPH absolute-scale theorem
for \(g_e\).
...
For realized same-label pullback data,
\[
q_e=\sqrt{g_e d_e},
\qquad
...
\begin{enumerate}
\item the proof-facing realized same-label pullback emission, preferably the full
\((g_e,d_e)\) package or at minimum the centered eta-class itself; and
\item the shared charged-lepton left basis needed to form physical PMNS mixing.
\end{enumerate}
...
remaining task is executed unquenched production data together with production statistical and
systematic control. The derivation nevertheless has a complete
...
\subsection{Surrogate execution bundle and open frontier}
...
\]
In the supplement, the same statement appears as the L\"uders update on the record algebra. The
observer paper uses the same fixed-cutoff Born/L\"uders package at theorem status.
...
important honesty condition in the paper. At the flavor-structural level, family
identity is transport-stable intrinsic structure first and named experimental family assignment
second.

The Yukawa/excitation dictionary therefore remains a central open task. The flavor
...
Particle identity in OPH is a refinement-stable
...
This chapter distinguishes two levels. The fixed-cutoff measurement interface is theorem-bearing
on the microphysics surface: central record algebra, Born probabilities, L\"uders conditioning,
and repeated-read stability are fixed-cutoff statements there. The stronger global
observer-continuation or strange-loop story is not proved at the same level, so the
particle paper uses the fixed-cutoff measurement package directly and leaves the larger
metaphysical closure story out of the technical chain.
...
This section addresses questions that lie outside the recovered-core and emitted-row portions of
the particle-spectrum derivation. Matter versus antimatter, supersymmetry, and several adjacent
questions use the structural branch as input but do not become emitted rows at the same level as
the bosonic and quark sectors.
...
The baryon asymmetry scale remains a Phase-III continuation, and suppression-counting estimates are
not a substitute for a derived out-of-equilibrium mechanism. The present derivation
does not expose a baryogenesis output row, so this section reports the existing OPH idea
without presenting a sharp matter/antimatter abundance prediction as a closed theorem or emitted output.
...
unification. Its point here is narrow: the realized OPH branch does not \emph{need} a
supersymmetric partner spectrum in order to explain why unification-like running might appear.
...
The technical answer to ``why is there no supersymmetry in the derived spectrum?'' is: the realized OPH branch stops at the Standard
...
branch. Three generations are structurally recovered; the full flavor dictionary,
...
Ordinary visible matter is dominated by hadrons, especially protons and neutrons. The OPH
derivation has a substantial hadron pipeline, but it still lacks promotable hadron rows on the
default surface. The route toward ordinary matter is therefore structurally present without yet
yielding the full observed matter spectrum numerically end to end.

\subsection{How to read these open questions}

A failure in one of these continuation stories would not by itself undo the
recovered-core gauge and gravity chain, nor would it erase the reported bosonic and quark
rows. A successful closure of these chapters would strengthen the particle-spectrum derivation
without changing the fact that they sit beyond the closed structural core.

\section{Conclusion}

OPH has a nontrivial particle-spectrum derivation, but it is not
...
The paper places the finished structural results, the emitted quantitative rows, and the
remaining closure fronts on one surface. The visible gates remain the same ones that
