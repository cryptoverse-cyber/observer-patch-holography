...
\]
The smaller object beneath that determinant-line route is a refinement-stable uncentered trace
lift.
...
\paragraph{Neutrinos.}
The weighted-cycle / Majorana-holonomy branch closes the PMNS/hierarchy shape and leaves no further discrete branch object on that lane. The normalized same-label overlap-defect weight section is already closed beneath that shape, so the remaining freedom is the positive rescaling orbit of the absolute spectrum together with the attachment bridge invariant above \(qbar_e\). Charged leptons and neutrinos are therefore open for different reasons: charged leptons miss promotion of \(\widehat C_e^{\mathrm{cand}}\), the refinement-stable uncentered trace lift, and then one affine-covariant absolute anchor \(A_{\mathrm{ch}}\), while neutrinos miss the bridge invariant that would emit one positive normalization scalar after the shape closure is already in hand.
...
Quark family & up quark & overlap continuation & reported value \(0.00228766~\mathrm{GeV}\); stronger two-scalar candidate \(0.00215992~\mathrm{GeV}\) & the current D12 sheet is a strict no-go for the physical CKM shell; emit the finite left-handed same-label relative-sheet orbit together with the provider/evaluator that exposes it, then the selector \(\sigma_{ud}\), then the mass-side scale law on the selected branch \\
...
\item the broader honest overlap-continuation frontier is a wrong-branch no-go on the current D12 sheet, so the exact next object is the finite same-label left-handed orbit together with the provider/evaluator that exposes it and then the discrete selector \(\sigma_{ud}\); only after that branch selection does the mass-side ray \(\mathcal R_{D12}^{ud}\) ask for its own intrinsic scale law.
\end{enumerate}
...
\]
The normalized same-label overlap-defect weight section \(qbar_e\) is already closed beneath
this step, so the remaining attachment gap is one positive bridge invariant above \(qbar_e\),
with selector-centered phase-cocycle triviality as the immediate gate on the finite-angle scalar
route.
