...
open, while hadrons remain execution-bound. The purpose of the paper is therefore not to claim full closure, but to give a concise
...
notation. In the active electroweak branch, the current-carrier chart and the older freeze-once
repair surface are retained as historical validation layers, while the public \(W/Z\) rows come
from a target-free source-only repair theorem. So the honest remaining question is no longer
whether the electroweak lane has a missing mass-side blocker. It is whether one wants to report
only the active closed theorem surface, or also keep the older validation layers visible as part
of the derivation history.

\subsection{What electroweak calibration implies}

The earlier calibration diagnostics are still scientifically useful because they show how the
closed surface was isolated. The implied-\(P\) comparison made clear that the older
current-carrier readout was not an exact common closure surface, so the real issue was a
transport/matching law on the electroweak stage rather than a lack of decimal places in the
declared pixel constant. That missing law is the point now promoted to the target-free
source-only repair theorem. The paper therefore describes three layers at once. First, one common
pixel constant \(P\) is the driver of the active public electroweak surface. Second, the older
current-carrier and freeze-once layers remain as validation surfaces. Third, the same layered
logic explains why the later flavor and neutrino lanes remain open even though the electroweak
lane is closed.
...
Electroweak calibration & \(W\) boson & calibration & \(80.37700001539531~\mathrm{GeV}\) from the target-free source-only repair theorem on the calibrated D10 source basis & no open D10 mass-side blocker; freeze-once surface retained only as validation \\
...
\]
so the mathematics keeps the three layers separate: the closed current-carrier chart, the
historical freeze-once validation surface above it, and the active target-free theorem surface.
...
\]
That freeze-once law is not the active public electroweak surface anymore. The promoted theorem is
the target-free source-only repair law.
...
\]
The older freeze-once coherent repair surface is retained only as compare-only validation, with
machine-scale deltas
\[
...
Historically, the source-only target-free problem was recorded on disk in three layers:
an underdetermination theorem for the quadratic repair family, the smallest conditional route
through \(\mathrm{ColorBalancedQuadraticRepairDescent}_{EW}\), and the strongest source-only
candidate \(\mathrm{EWTargetEmitter}_{D10}\). Those layers are still useful as scaffolding, but
they are no longer the active public electroweak surface.
...
The electroweak chapter therefore has to keep the active closed theorem surface and the older
validation layers visible at once: the reported target-free source-only rows, the current-carrier
chart beneath them, and the freeze-once repair surface retained for compare-only validation. That
is the technically honest way to present the bosonic sector.
