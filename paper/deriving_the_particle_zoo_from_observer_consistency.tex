...
\begin{abstract}
Observer-Patch Holography (OPH) asks how much of the particle spectrum can be derived from one shared local input rather than from a long fitted parameter list. Building on the OPH recovery of relativity and Standard Model structure, this paper develops the particle branch from the dimensionless pixel constant \(P \equiv a_{\mathrm{cell}}/\ell_P^2\). It fixes the structural massless carriers, closes a target-free \(W/Z\) lane, carries a quantitative Higgs/top branch, closes the exact PDG 2025 API running-quark sextet together with explicit exact forward Yukawas on the selected public quark frame class \(f_P\), and gives a theorem-grade weighted-cycle neutrino family in the observed hierarchy and mixing regime. The exact non-hadron surface includes exact structural zeros, an exact frozen \(W/Z\) pair, exact same-family charged witnesses, the selected-class exact quark theorem together with its supporting exact surfaces, and exact neutrino-side readout objects. Charged-lepton absolute closure, a global classification of all quark frame classes, flavor-labeled neutrinos, and hadrons remain explicit boundary objects.
\end{abstract}
...
the selected-class exact quark theorem, and the weighted-cycle neutrino theorem branch are emitted, while charged leptons, flavor-labeled neutrinos, and
...
exact centered readback and a closed common-shift no-go, then waits on promotion of
...
rows. Charged-lepton, fuller flavor-labeled neutrino, and hadron closure theorems sit beyond the
...
flavor-labeled neutrinos, and hadrons have derivation chains beyond that surface, and the
charged-lepton and hadron lanes terminate at named constructive objects rather than promotable
masses.
