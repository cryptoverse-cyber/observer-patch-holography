#!/usr/bin/env python3
"""Sweep the residual Majorana phase plane and emit stability / envelope bounds."""
...
def main() -> int:
    ...
    ap.add_argument("--out", default="runs/neutrino/majorana_phase_envelope.json")
    ap.add_argument("--grid", type=int, default=33, help="Residual-basis sweep resolution per axis.")
    args = ap.parse_args()

    capacity_path = ROOT / args.capacity if not pathlib.Path(args.capacity).is_absolute() else pathlib.Path(args.capacity)
